 ...
 *   리그는 셋(SPL · IPL · 10mountain)뿐이고 새 리그는 관리자가 만들 때만 생긴다.
 *   시즌 목록은 시즌이 열리고 닫힐 때만 바뀐다 — 하루에 한 번도 안 바뀌는 값이다.
 *   1시간 늦게 반영돼도 «틀린 화면» 이 되지 않는 것들만 여기 둔다.
 */

/*
 * ⚠ **값을 크게 올렸다 (2026-09-01 오후 · D-240 후속).** 실측 근거가 있다.
 *
 * 캐시가 비는 **그 한 순간**에 들어온 사람이 500 을 본다. 실측:
 *
 * ```
 * 500  cache=MISS  age=0     ← 캐시가 비었고, 그때 DB 가 수집에 눌려 있어 죽었다
 * 200  cache=MISS  age=0     ← 다시 시도해서 성공
 * 200  cache=HIT   age=7 · 14 · 21 · 28 …   ← 이후는 전부 빠르다
 * ```
 *
 * `stale-while-revalidate` 가 있는데도 500 이 나는 이유는 **만료가 아니라 축출**이기
 * 때문이다 — 항목 자체가 사라져 `MISS` 가 되면 내줄 낡은 값도 없다.
 *
 * ── 왜 워밍에 기대지 않는가
 *   5분마다 데우는 워크플로(`cache-warm.yml`)를 만들어 올렸는데
 *   **GitHub 이 아직 한 번도 돌리지 않았다** (수집 잡만 돈다). 파일·문법은 정상이다.
 *   **남이 돌려 주기를 기다리는 구조에 안정성을 걸 수 없다.** 그래서 우리 쪽에서 줄인다.
 *
 * ── 그래서 무엇을 잃나
 *   랭킹·기록이 **최대 30분** 늦게 갱신될 수 있다.
 *   경기 하나가 30분 늦게 반영되는 것과, 첫 방문자가 500 을 보는 것 중 **후자가 나쁘다.**
 *   DB 가 안정되면 다시 내린다 — 이 값은 **임시 조치**이고 그 사실을 여기 적어 둔다.
 */
const CACHE_LONG_SECONDS = 21600
 ...
 */
const CACHE_RECORD_SECONDS = 1800
 ...
 */
/**
 * ⚠ **15 → 60 (2026-09-01 저녁, 오픈 당일)**
 *
 * 15초로는 못 버텼다. 배포 뒤에도 이 경로만 계속 새어 나갔다 —
 * `18:00 · 18:04 · 18:11 · 18:32` 전부 `/api/eggs/broken` 이다.
 * 15초마다 한 번씩 DB 로 내려가는데, 수집이 DB 를 물고 있으면 그 한 번이 죽고
 * 그때 엣지에 채울 것이 없는 상태가 만들어진다.
 *
 * **오픈 당일에는 안정이 먼저다.** 60초로 올린다.
 * 알을 깬 사람이 최대 1분 늦게 보는 대신, 아무도 500 을 안 본다.
 * `stale-while-revalidate` 덕에 만료 직후 갱신이 붙으므로 실제 지연은 대개 그보다 짧다.
 *
 * D-222 ⑤(「깬 직후에 안 보이면 안 깨진 줄 안다」)는 **여전히 옳다.**
 * DB 가 안정되면 되돌린다 — 되돌릴 값은 15 다.
 */
const CACHE_EGG_SECONDS = 60
