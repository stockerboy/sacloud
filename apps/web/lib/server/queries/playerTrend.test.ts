...
describe('선수 추이 — 사장님 지시서 (2026-09-10 · 2026-09-11 정정)', () => {
  ...
  it('게임이 없던 날은 0 으로 이어지고, 미래는 future 다', () => {
  ...
  it('첫 경기가 9/7 이면 9/3~9/6 은 0%, 9/7 부터 실제값 · 경기마다 직후 값이 points 에 담긴다', () => {
    const rows = [
    ...
    expect(days[4]?.points.map((p) => p.win_rate)).toEqual([100, 50])
    expect(days[4]?.points[0]?.at).toBeCloseTo(13 / 24, 2) // KST 19:00 → 06:00 부터 13시간
  })

  it('경기 없는 날은 전날 값을 그대로 잇는다 (0 으로 떨어뜨리지 않는다)', () => {
    const rows = [
      { startAt: at('2026-09-05T10:00:00Z'), winnerSide: 'red', side: 'red', kill: 10, death: 5 },
      { startAt: at('2026-09-05T11:00:00Z'), winnerSide: 'red', side: 'red', kill: 10, death: 10 },
    ]
    const days = buildPlayerTrend(rows, at('2026-09-10T12:00:00Z'))
    expect(days[2]?.win_rate).toBe(100)
    expect(days[3]?.win_rate).toBe(100)
    expect(days[3]?.kd).toBe(57.1)
    expect(days[3]?.games).toBe(0)
  })

  it('하루 1판은 «안 찍힘» — 전날 값에 머물고, 누적에는 들어간다', () => {
