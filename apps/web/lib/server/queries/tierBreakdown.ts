...
/**
 * ★티어를 「지금 명부」로 본다★ (2026-09-10). `false` 면 옛 방식
 * (`MatchPlayerStat.opponentDivisionAtMatch` · 경기 당시 스냅샷)으로 돌아간다.
 * 까닭은 아래 `tierOf` 주석에 실측과 함께 적어 뒀다.
 *
 * 타입을 `boolean` 으로 넓힌 이유는 리터럴로 좁히면 옛 가지가 «닿을 수 없는 코드» 가 되기 때문이다.
 */
const TIER_FROM_CURRENT_DIVISION: boolean = true
...
interface TierBucket {
  ...
  win: number
  lose: number
  /** ★킬뎃을 아는 판수★ — `games` 와 다르다 (D-149). 이게 킬뎃의 분모다 */
  knownGames: number
  kill: number
  death: number
...
async function tiersOf(
  ...
  /* 클랜명은 표기용이라 한 번만 읽는다. ★티어도 같이 읽는다★ — 아래 `tierOf` 가 쓴다 */
  const clanRows =
    opponentIds.size === 0
      ? []
  ...
  /**
   * ★어느 티어로 셀 것인가★ — 2026-09-10 에 바뀌었다.
   *
   * ⚠ ★옛 방식(`opponentDivisionAtMatch`)을 지금 쓰면 화면이 거짓말을 한다.★ 실측 —
   *   ```
   *   저장된 「경기 당시 상대 티어」  1~6      ← ★옛 6티어 체계★
   *   지금 IPL 티어                  1~3      ← 사장님이 2026-09-10 에 셋으로 확정
   *   ```
   *   그대로 두면 ① 4·5·6 으로 찍힌 ★13,040줄(56%)이 어느 줄에도 안 들어가 사라지고★
   *   ② 남은 1~3 줄에 `ASTRA`·`CHALLENGER1` 이라는 ★새 이름이 붙는데 뜻이 다르다.★
   *   게다가 앞으로 들어올 경기는 1~3 으로 찍혀 ★한 칸에 두 체계가 섞인다.★
   *
   *   티어가 바뀐 것은 승격·강등이 아니라 ★체계를 통째로 다시 그은 것★ 이다.
   *   랭킹 점수도 이미 ★지금 티어★ 로 매긴다 (`iplRankApply` 의 `TIER_OF`).
   *   화면 두 곳이 다른 답을 내지 않게 ★여기도 지금 티어를 본다.★
   *
   * ⚠ ★DB 는 한 칸도 안 고쳤다.★ `opponentDivisionAtMatch` 는 그대로 있고
   *   아래 스위치를 `false` 로 두면 옛 방식이 그대로 돌아온다 (`CLAUDE.md` 1-4).
   *   래더 계산(`CLAUDE.md` 6장 4번의 「경기 당시 스냅샷」)은 ★손대지 않았다.★
   */
  const tierOf = (row: (typeof rows)[number]): number => {
    if (!TIER_FROM_CURRENT_DIVISION) return row.opponentDivisionAtMatch
    const opponentId = row.side === 'red' ? row.match.blueLeagueClanId : row.match.redLeagueClanId
    /* 지금 명부에 없는 상대(리그를 떠난 클랜)는 ★지어내지 않고★ 옛 값으로 떨어진다 */
    return divisionById.get(opponentId) ?? row.opponentDivisionAtMatch
  }
