...
/**
 * ⚠ ★옛 화면이다. 지금은 기본값이 아니다★ (2026-09-10 사장님 지시로 뒤집혔다).
 *
 * > «여유되면 클랜랭킹까지 매기고 지금 클랜랭킹페이지에 클랜들이 그냥 나열만 돼있음»
 *
 * 2026-09-02(D-260)의 「고용가능 클랜」 — ★순위 없이 이름 가나다순★ 목록이다.
 * 아래 본문은 ★그때 그대로다. 한 줄도 고치지 않았다★ (`CLAUDE.md` 1-4).
 *
 * ── 되돌리는 법
 *   `ClanDirectory.tsx` 맨 위의 `RANKED` 를 `false` 로 둔다. 그러면 이 화면이 다시 나온다.
 */
