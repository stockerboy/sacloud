...
const SPL_NOTES: readonly { title: string; lines: readonly string[] }[] = [
  {
    ...
    lines: [
      ...
      '전환 비용은 시즌 0(9/3~10/1) 동안만 무료입니다.',
  ...
  },
  {
    title: '리그 참가 신청',
    lines: [
      '시즌 1 에 참가할 IPL 리그 참가 클랜을 시즌 0 동안 모집합니다.',
      '이 기간에는 PL → IPL 전환 클랜을 제외한 신규 참가 클랜에 5만원의 등록 비용이 발생합니다.',
      '정규 시즌 시작은 10월 1일입니다. 참가 신청은 10월 15일까지 받습니다.',
    ],
  },
]
      ...
      */}
      <div className="py-[var(--section-gap)] max-md:pb-8 max-md:pt-[16px]">
        ...
        */}
        <RankHeader
          /*
           * ⚠ ★2026-09-22 밤 — 제목을 되살렸다★ (사장님: 「서플라이랑 똑같이」).
           *   9/16 에 뺀 까닭은 「바로 위 본문 탭에 같은 말이 굵게 서 있어서」 였다.
           *   그 본문 탭은 오늘 ★고정 띠★ 로 올라갔고, 서플라이는 ★띠의 주황 탭과
           *   본문 제목을 둘 다★ 둔다 (실측: 제목 21px/400 검정 · x=219).
           *   ★되돌리려면 빈 문자열로 두면 된다★ — 그러면 여백도 같이 준다.
           */
          title="클랜랭킹"
          /*
           ...
           */
          /*
           * ⚠ ★2026-09-21 — C1 은 잣대가 다르다★ (사장님이 화면을 보고 «b» 로 고르심)
           *
           *   C1 은 ★열 클랜이 자기들끼리만★ 붙어서 Elo 가 수렴하지 않는다
           *   (열 곳이 177점 안에 뭉쳤다 — 그래서 48% 가 1위였다).
           *   그래서 C1 만 ★승률을 판수로 누른 값★ 으로 센다 (`clanRankByOf`).
           *   ★화면 문구도 그에 맞춰야 한다★ — 안 그러면 화면이 거짓말을 한다.
           */
          notice={
            leagueSlug === 'c1'
              ? '승률이 높은 순입니다. 다만 판이 적으면 그만큼 눌립니다 — 스무 판 남짓으로 높은 승률을 낸 클랜이 수백 판을 뛴 클랜을 쉽게 넘지 못합니다.'
              : byTier
