 ...
 *
 * 화면 코드는 **한 글자도 안 바뀌었다** — `LeaguePlayerRecordScreen.tsx` 가 그대로 그 파일이다.
 ...
 * ```
 *
 * ⚠ **재수출(`export default LeaguePlayerRecordPage`)로는 안 된다.** 그러면 Next 가 이 파일을
...
export function generateStaticParams(): { leagueSlug: string; playerId: string }[] {
  return []
}
...
export default function Page({ params }: { params: Promise<{ leagueSlug: string; playerId: string }> }) {
  return <LeaguePlayerRecordPage params={params} />
}
