 ...
 *
 *      ⚠ **2026-09-02 — 기록실(경기 목록)에도 창이 걸리게 바뀌었다** (결함 #32 · 사장님 지시).
 *      옛 서술은 «기록실에는 **그대로 남는다**» 였다. 아래 마지막 테스트에 경위가 있다.
 *
...
describe.runIf(up)('창 밖 경기는 성적에서 빠지고 기록실에는 남는다 (D-175 정한 것 ②)', () => {
  ...
  /*
   * ⚠ **2026-09-02 뒤집혔다 (결함 #32 · 사장님 지시)**
   *
   * 옛 규칙 (D-175 정한 것 ②) — «기록실(경기 목록)에는 창을 **안 건다.** 옛 기록은 거기서
   * 계속 보인다». 이 테스트는 그 규칙을 지키고 있었다.
   *
   * 사장님: *"시즌이 7/1부터인데 기록이 … 내가 짜르라고 했잖아"* — 선수 기록실에 6월 3일
   * 경기가 「3달 전」 카드로 그대로 나왔다. 성적 수치는 이미 창이 걸려 있었고 **목록만**
   * 전 기간이라 숫자와 카드가 어긋나 보였다.
   *
   * 그래서 `matchPage` 가 `withSeasonWindow` 를 걸게 됐다
   * (`apps/web/lib/server/queries/matches.ts` 494행 주석에 경위가 있다).
   * **코드가 맞고 이 테스트가 낡은 것이다.** 규칙이 바뀐 쪽으로 고친다 —
   * 옛 기대값은 위 주석에 남긴다.
   *
   * 데이터는 지우지 않았다. 화면이 거를 뿐이고 과거 시즌 조회는 별도 기능이다.
   */
  it('경기 목록(기록실)도 창 밖 경기를 뺀다 — 숫자와 카드가 어긋나지 않게 (결함 #32)', async () => {
    const page = await getLeaguePlayerMatches(leagueId, playerId, null, 20)
    const ids = page?.items.map((row) => row.id) ?? []
    /* 창 밖 미러는 목록에서도 빠진다 (옛 규칙에서는 보였다) */
    expect(ids).not.toContain(`${P}mirror-out`)
    /* 만든 4건 중 창 안 3건만 남는다 — 래더 판정은 여전히 안 건다
       (`other-in` 은 래더 밖 origin 인데도 목록에는 보인다) */
    expect(ids.length).toBe(3)
    expect(ids).toContain(`${P}other-in`)
  })
})
