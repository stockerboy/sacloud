...
/**
 * 글 삭제 확인 **화면** (지시 #14-2 로 라우트에서 분리).
 * 비로그인 글은 작성 시 정한 비밀번호를 입력해야 삭제된다 (원본 동작).
 * 삭제되면 목록(`basePath`)으로, 취소하면 글(`basePath/{id}`)로 간다.
 * 본문은 `app/board/[category]/[id]/delete/page.tsx` 그대로.
 */
export function BoardDeleteScreen({ id, basePath }: { id: string; basePath: string }) {
  ...
  return (
    <div className="rounded-[var(--radius)] border border-line bg-card px-6 py-6 text-text max-md:px-4">
      ...
      <div className="mt-6 flex gap-2">
        ...
        <Link href={`${basePath}/${id}`} className="btn-line h-10 w-24 text-sm">
          취소
        </Link>
      </div>
    </div>
