 ...
 * ★육각 점수의 기준점★ — 3000.
 *
 * ⚠ ★2026-09-20 — 한 번 0 으로 바꿨다가 되돌렸다.★
 *   사장님이 「래더 3000점 부터 하지말고 그냥 0점부터 계산해」 라고 하신 그 「래더」 는
 *   ★화면에 「래더 n점」 으로 나오는 `LeaguePlayer.rating`★ 이지 이 값이 아니다.
 *   이 값(`LeaguePlayerHex.score`)은 ★육각 순위를 매기는 속값★ 이라 화면에 안 나온다.
 *   ★엉뚱한 값을 건드릴 뻔했다.★ 0 기준은 화면 쪽에서 처리한다.
 */
