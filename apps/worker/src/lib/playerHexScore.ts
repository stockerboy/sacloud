...
/**
 * ★★기준점 — 0 이다★★ (2026-09-20 사장님: 「래더 3000점 부터 하지말고 그냥 0점부터 계산해」)
 *
 * ── 왜 0 인가
 *   3000 은 ★아무 뜻이 없는 숫자★ 였다. 「3,058점」 을 보면 잘한 건지 못한 건지
 *   알려면 3000을 빼야 한다. 0 이면 ★부호가 곧 답★ 이다 —
 *   ```
 *     +58점   평균보다 잘한다
 *       0점   딱 평균
 *     −41점   평균보다 못한다
 *   ```
 *
 * ⚠ ★옛값은 3000 이었다★ — 되돌리려면 이 한 줄이다 (CLAUDE.md 1-4).
 * ⚠ 바꾸면 ★모든 선수 점수가 3000씩 내려간다.★ 재계산을 돌려야 화면에 반영된다.
 */
export const HEX_BASE = 0
/** ★옛 기준점★ (2026-09-10 ~ 2026-09-20) */
export const HEX_BASE_V1 = 3000
export const HEX_SPREAD = 700
/**
 * ★판수 무게★ — 라운드가 이만큼이면 점수의 절반을 받는다.
 *
 * ── 이력 (두 번 올렸다. 둘 다 사장님이 같은 것을 지적하셨다)
 *   2026-09-12  120 → 300   «지금 판수적은데 상위권인 애들이 너무 많아»
 *   2026-09-20  300 → ★600★  «이사람 왜 24판밖에 안하고 이렇게 상위권이냐..?»
 *
 * ── 왜 300 으로 모자랐나 (2026-09-20 실측 · IPL 스나 상위 10명)
 *
 *     3297점 · ★18판★ hydraIisk   ← 3위
 *     3225점 · ★11판★ 야옹힝       ← 10위
 *
 *   한 판이 12~16라운드다. 11판이면 150라운드쯤이라 300에서는
 *   ★수축이 0.33★ 밖에 안 됐다 — 성적이 좋으면 그걸 뚫고 올라온다.
 *   600이면 같은 사람이 0.2 로 내려가 ★10위권 밖★ 이 된다.
 *
 * ── 600을 고른 근거 (사장님이 표를 보고 고르셨다)
 *
 *     K=300   상위10 중 25판 미만 ★2명★
 *     K=600   상위10 중 25판 미만 ★1명★ · 136판 두부젤이 8위→5위로 올라옴
 *     K=900   같은 1명이지만 ★점수 폭이 너무 좁아진다★ (1위가 3240)
 *
 *   ⚠ 600은 ★판수를 더 무겁게 볼 뿐 문을 닫는 것이 아니다.★ 적게 뛰어도
 *     아주 잘하면 올라올 수 있다 — 다만 ★훨씬 잘해야 한다.★
 */
export const HEX_SHRINK_K = 600
/** ★옛 값★ (2026-09-10 ~ 2026-09-12) */
export const HEX_SHRINK_K_V1 = 120
/** ★옛 값★ (2026-09-12 ~ 2026-09-20) — 이것도 판수 적은 사람을 다 못 막았다 */
export const HEX_SHRINK_K_V2 = 300
/**
