...
/*
 * ⚠ ★2026-09-21 낮 — `elo` 로 되돌렸다★ (사장님: 3rd.supply 3부리그를 학습하라)
 *
 *   원본 3부리그 클랜랭킹을 실제로 봤다 —
 *   ```
 *   1 -tsAr.nTc 2,093점   2 hingˇ 1,991점   3 너구리마을 1,981점 …
 *   「랭킹은 1시간마다 갱신되며, ★배치고사가 종료된 클랜만★ 표시됩니다」
 *   ```
 *   ★승률도 판수도 안 보여 준다. 래더 점수 하나로 줄을 세운다.★
 *   즉 원본은 ★Elo★ 다 — 내가 새벽에 만든 「기록순」 은 지어낸 것이었다.
 *
 *   개인 쪽도 실측으로 확인했다 (`sourceRating`·`sourceRatingDelta` 2,940줄) —
 *   ```
 *   이기면 +0~+25 (평균 +12.9) · 지면 -19~0 (평균 -11.9)
 *   1791점이 이기면 +17 · 2570점이 이기면 +10   ← 약한 쪽이 더 받는다
 *   ★킬뎃은 증감에 안 들어간다★ (5킬8뎃 -11 · 13킬10뎃 -11)
 *   ```
 *
 *   ★옛 「기록순」 은 지우지 않았다★ — `'record'` 로 두면 그대로 돌아온다.
 */
