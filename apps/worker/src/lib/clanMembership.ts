...
/**
 * ── `판정보류` 는 **점수를 반영한다** (2026-08-31 · 지시에 없던 칸을 우리가 정했다)
 *
 * 사용자 지시는 두 칸만 정했다 — `정상`(반영) 과 `열산의심`(보류·0점).
 * 소속을 몰라 판정 못 하는 `판정보류` 를 어떻게 할지는 정해지지 않았다.
 *
 * **보류로 처리하면 IPL 전체가 얼어붙는다.** 소속 관측은 2026-08-31 부터 쌓이기 시작하므로
 * 그 이전 경기는 거의 전부 `판정보류` 다.
 *
 * 그래서 `판정보류` 는 **점수를 반영하고 딱지도 붙이지 않는다.**
 * 이 규칙은 예외를 잡는 장치이지 관문이 아니다 — 근거 없이 의심하면 멀쩡한 경기에
 * 딱지가 붙는다. 나중에 소속을 알게 되면 그때 재판정한다 (되돌릴 수 있다).
 *
 * ⚠ 사용자가 다르게 정하면 이 주석과 호출부를 함께 고친다.
 */
