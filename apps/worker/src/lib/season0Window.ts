 ...
 * ── 창
 *
 *   시작   ★2026-09-03 07:00 (KST)★ = 2026-09-02T22:00:00Z   (2026-09-04 에 옮겼다)
 *   끝     ★2026-10-01 00:00 (KST)★ = 2026-09-30T15:00:00Z   ← 10월 첫째 목요일
 *
 *   ⚠ ★옛 값은 7/1 시작 · 끝 없음 이었다★ (`SEASON0_FROM_V2` · `SEASON0_TO_V2` 로 남겼다)
 *
...
/**
 * ★옛 시작값 (2026-07-01 KST)★ — 2026-09-04 이전. ★지우지 않는다★ (`CLAUDE.md` 1-4).
 *
 * 그때는 「7/1 부터 시즌0」 이었다. 아래 `SEASON0_FROM` 이 그것을 대신한다.
 */
export const SEASON0_FROM_V2 = new Date('2026-06-30T15:00:00.000Z')

/**
 * ★★2026-09-03 07:00 (KST)★★ — DB `startAt` 은 UTC 이므로 9시간 뺀 값이다.
 *
 * ══ ★왜 여기로 옮겼나★ (2026-09-04 사장님)
 *
 * > «IPL은 전 기록 다 버리고 그냥 ★9월3일 오전 7시를 기준으로 그 이후의 기록만★ 전부 기록한다
 * >  그걸 ★시즌0★ 이라고 한다» · «열산은 9/3일 오전 7시 전 기록은 전부 다 버린다»
 * > «SPL 은 ★9/3일 오전 7시 이후 기준으로 기록을 실시간으로 받고 있는 걸 시즌0★ 으로 하고 계속 기록한다»
 *
 * ★그 전 기록은 지우지 않는다. 화면에서 안 보이게 할 뿐이다★ (사장님 선택 B).
 * ★래더는 전원 3000점부터 새로 시작한다★ (사장님 선택 A).
 *
 * ⚠ ★이 값 하나가 화면과 계산 양쪽을 움직인다★ —
 *   `apps/web/.../season0Scope.ts` 가 이 파일을 ★경로로 직접 읽는다★ (값 복제 없음).
 *   ★그래서 여기만 고치면 된다.★ 다른 데 같은 날짜를 또 박지 마라.
 */
export const SEASON0_FROM = new Date('2026-09-02T22:00:00.000Z')
 ...
 * 몇 번을 돌려도 같은 값이 나온다.
 */
export const SEASON0_TO: Date | null = new Date('2026-09-30T15:00:00.000Z')

/**
 * ★옛 끝값 (열린 구간)★ — 2026-09-04 이전. ★지우지 않는다★ (`CLAUDE.md` 1-4).
 *
 * 그때는 「시즌1 오픈일을 사용자가 정한다」라서 끝이 없었다.
 * ★이제 정해졌다 — 10월 첫째 목요일(2026-10-01 00:00 KST)★ 이다.
 */
export const SEASON0_TO_V2: Date | null = null
