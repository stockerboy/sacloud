...
/* ========================================================================== */
/* ★★기록을 「넣는」 창 — 집계 창과 다르다★★ (2026-09-04)                      */
/* ========================================================================== */

/**
 * **IPL 경기를 `Match` 로 투영할 때의 시작점** — 2026-03-05 00:00 (KST).
 *
 * ⚠ **`SEASON0_FROM` 과 섞지 마라.** 둘은 하는 일이 다르다.
 *
 * ```
 * SEASON0_FROM (7/1)     ★집계 창★ — 래더·시즌 누적을 어디서부터 셀까
 * IPL_PROJECT_FROM (3/5) ★적재 창★ — 경기를 표에 넣을까 말까
 * ```
 *
 * ── 왜 갈랐나
 *
 * 2026-09-04 실측: **4·5·6월 IPL 경기가 화면에 하나도 없었다** (4월 0건 · 5월 0건 · 6월 130건).
 * 배틀로그 원문은 **607건이나 이미 받아 놨는데** `Match` 가 없어서 **보이지 않았다.**
 * `iplProject` 가 `SEASON0_FROM` 을 적재 기준으로도 써서 **7/1 이전을 통째로 버렸기** 때문이다.
 *
 * 그런데 **집계 창을 3/5 로 내리면 안 된다** — 시즌 구분이 이렇게 확정돼 있다:
 *
 * ```
 * legacy  ~3/4        Beta  3/5~7/1        시즌0  7/2~9/30        시즌1  10/1~
 * ```
 *
 * **3/5~7/1 은 Beta 지 시즌0 이 아니다.** 집계 창을 내리면 **Beta 경기가 시즌0 래더에 섞인다.**
 *
 * 그래서 **적재는 3/5 부터, 집계는 7/1 부터**로 갈랐다.
 * 기록실은 원래 창으로 거르지 않으므로(`SEASON0_FROM` 머리말) **3~6월 경기가 화면에 나온다.**
 *
 * ⚠ **되돌리려면 이 값을 `SEASON0_FROM` 으로 바꾸면 된다.** 그러면 옛 동작이다.
 */
export const IPL_PROJECT_FROM = new Date('2026-03-04T15:00:00.000Z')

/**
 * **옛 적재 시작값** — `SEASON0_FROM`(7/1) 을 그대로 썼다. 지우지 않는다 (`CLAUDE.md` 1-4).
 *
 * 이 값으로 돌리면 **4~6월이 다시 화면에서 사라진다.**
 */
export const IPL_PROJECT_FROM_V1 = SEASON0_FROM
