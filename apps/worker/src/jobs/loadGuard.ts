...
/**
 * ★health 를 이만큼 연속으로 «못 쟀으면» 한 번은 돌아 본다★ (2026-09-20).
 *
 * ── 왜 필요한가
 *   `measureOnce` 는 못 재면 `stop` 을 돌려준다 — 「모르면서 두드리지 않는다」 는
 *   맞는 규칙이다. 그런데 그 바람에 ★수집이 12시간 동안 한 번도 안 돌았다.★
 *   실측 (2026-09-19~20):
 *   ```
 *     09-19 14:35  마지막 원문
 *     그 뒤 12시간  매 회차 「시작 전부터 무겁다」 · 새 원문 ★0건★
 *     그때 DB 는 눌려 느렸을 뿐이고 넥슨은 ★한 번도 안 막았다★
 *   ```
 *   ★모르는 것과 나쁜 것은 다르다.★ 모르는 채로 영영 안 도는 것이 더 나쁘다.
 *
 * ⚠ ★잰 값이 있는데 느린 경우는 그대로 물러난다.★ 여기서 뚫는 것은
 *   ★한 번도 못 잰 경우★ 뿐이다.
 */
export const BLIND_MAX = 3
