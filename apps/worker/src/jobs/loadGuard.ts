...
async function measureOnce(healthUrl: string, state: GuardState): Promise<Verdict> {
  ...
  let ms = 0
  try {
    const r = await new Promise<{ status: number; body: string; ms: number }>((resolve, reject) => {
      const child = spawn(
        'curl',
  ...
  try {
    dbStatus = (JSON.parse(body) as HealthShape).checks?.db?.status ?? null
  } catch {
    dbStatus = null
  }
...
export async function checkLoad(healthUrl: string, state: GuardState): Promise<Verdict> {
  ...
  /* 상태가 나쁜 것(200 아님 · db 아님)은 다시 재도 같다 — 시간 때문에 느린 경우만 다시 잰다 */
