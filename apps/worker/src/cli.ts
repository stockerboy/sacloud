...
★같은 이름 다른 클랜이라 뺀 이름★ ${out.ambiguousNames.join(' · ')}`)
    ...
    case 'barracks-clan-search': {
      ...
      */
      ...
      const r = await barracksBrowser().call('POST', `/api/Search/GetSearchClanAll/${encodeURIComponent(name)}/1`, '{}')
    ...
    case 'barracks-collect': {
      ...
      if (healthUrl) {
         ...
         *
         * 이 한 번은 ★언제나 제일 차가운 값★ 이다 (서버가 자고 있다).
         * 그걸 세어 두면 판 안에서 한 번만 더 느려도 ★두 번 연속★ 이 되어
         * 목록 수집이 통째로 끊긴다. 실측: 클랜 120곳 중 ★0곳★ 만 물어보고 끝났다.
         *
         * ⚠ ★`stop` 판정은 위에서 이미 했다.★ 여기서 푸는 것은 ★연속 횟수뿐★ 이고
         *   판 안에서 두 번 연속 느리면 그때는 정상적으로 쉰다.
         */
        state.slowStreak = 0
      }
