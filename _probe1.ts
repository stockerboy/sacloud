...
export function run(seed:number,pop:number,clanN:number,cutoff:number|undefined,perf:number,cap:number){
  ...
  return {board:personalLeaderboard(season,players,CANDIDATE2_DISPLAY,90*24*60,true),players,clans,season,matches}
}
const t0=Date.now()
...
for (const c of [undefined,0.92,0.90]){
  ...
  console.log('cutoff',String(c??'none').padStart(4),'| 실력상관',skillCorrelation(board).toFixed(3),'| 승률',d.winRate.toFixed(3),'| KD',d.kd.toFixed(3),'| 판수',d.games.toFixed(3),'| 최고',Math.round(board[0]!.displayed),'|',bands.map(b=>b+':'+board.filter(r=>r.displayed>=b).length).join(' '))
}
