...
```
1) A 와 사장님이 O-001 을 정한다             ← 지금 여기
2) 1단계 감속 마무리 — 문서 40개를 3개로, CLAUDE.md 534줄을 줄인다
3) 2단계 「눈」 — status 명령 하나 (마지막 경기 / 어제 대비 / 킬데스% / 출처 / 오류)
```
...
5  화면 5개 밖의 기능을 새로 만들지 않는다. 새 아이디어는 ORDERS.md 대기 칸으로
