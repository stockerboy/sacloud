...
- ⛔ ★00:50 수집기(autocollect) 멈춤 · 07:00 자동 재개★ — 주기 10분으로도 대기열 질의(8분)가 게시판·경기상세를 또 멈춰서. 크론에 `#PAUSED` · 재개 줄 `# RESUME-COLLECT`(9/27 07:00). 옛 표 `/root/crontab.bak-20260927-0050`.
  그 사이 새 경기·경기분석은 안 들어온다. 최근 이틀치로 좁혀도 21만 줄 · 최근 줄은 fetchCount 갱신으로 보임표시가 계속 깨져 색인만으로 못 읽는다 — 아침에 설계부터.
- ⛔ 00:53 정규화·라인업·season0-apply 도 `#PAUSED` (같은 07:00 재개 줄이 전부 푼다) · 옛 표 `/root/crontab.bak-20260927-0053`.
- ★진짜 병목 = DB 등급★: shared_buffers 256MB · max_connections 60 → Supabase ★Micro(1GB)★ 로 보인다. DB 7.6GB.
  배치를 전부 멈춘 조용한 DB 에서도 `matchKey` 범위 COUNT 가 10초를 넘겼다 — 디스크 읽기 한도(버스트 소진 추정 · [미확인])에 걸려 있다.
  무엇을 돌려도 게시판·경기상세가 같이 느려지는 이유. 4GB(Medium) 로 올리면 캐시 4배 + 디스크 한도 상향.

- ✅ ★01:06 DB Medium(4GB) 업그레이드★(사장님 · 결제한도도 풂) → shared_buffers 1GB · 접속 120. VACUUM 20초(보임표시 100%).
  수집 대기열 V1 45초→18.6초 · `pendingPairs`(JSON 안 읽는 판) 7.1초로 켬(9307987c · 351쌍 동일) · VPS 파일 반영. 크론 전부 재개(주기 10분 유지 · 옛 표 `/root/crontab.bak-20260927-0112`).
- ✅ 게시판(a146e3e3): 자유 1순위·인기 오른쪽 · 자유 맨 위 핫 4개 최신순(`hot_pinned` · HOT 표식) · 메인 핫 8개 · 더보기→자유.
  지금 살아 있는 글 중 인기 기준(좋아요5·댓글5) 넘은 글이 0개라 핫 칸이 비어 보인다 — 기준 넘은 13개는 전부 삭제된 글.
...
- 감시에서 반복 잡히던 25~51초 질의(`iplClanNumber.ts` 「SELECT DISTINCT subject, rawClanNo」) 원인 확정 — 쿼리 사정이 아니라 ★이 테이블 last_vacuum 이 도대체 한 번도 안 돐★(자동백업 기준치를 안 넘는 dead tuple 11만줄)로 플래너가 맞는 부분인덱스(`BCMR_subject_clanno_idx`) 대신 더 큰 인덱스를 고르고 heap fetch 13만 번.
  세션 풀러(5432)로 직접 `VACUUM (ANALYZE)` 한 번 돌림(48초) → 같은 질의 ★51,130ms → 820ms★(63배). 코드 변경 없음 — DB 유지보수만으로 해결.
- 상단바 CPL(Supply2.0) 로고 되돌림(a01183cd) — 날개 문양은 서랍 「경쟁전」 묶음에만, 상단바는 옷 로고로.
- 사장님이 개인랭킹 「순위중복」(같은 닉네임 두 번) 보고 — 검증: API 직접 조회로 1·2쪽 40행 전부 대조, ★실제 중복 0★.
  4위 「준퇹」(pid cmtlfuxw…, 183전 63.4%)과 31위 「준푸잉」(pid cmtleqp6c…, 156전 52.6%)은 ★서로 다른 사람★ — 품/혽/푸 음절이 작은 글자에서 거의 구별 안 되는 우연. 데이터 버그 아님.
...
- ★상단바 CPL 로고(GNB_CPL_LOGO)도 Supply1.0 과 같은 날개 문양으로★ (사장님이 같은 그림을 다시 주며 「경쟁전 마크 이거로 바꿘줘」) — `leagueLogo.ts` cpl 항목을 `league-supply1.webp` 로(새 파일 만들지 않고 같은 파일 공유). 옷 깨진유리 CPL 로고 파일은 그대로 남음.
- 라운드 그래프 마크 고정 재확인 — 헤드리스 탐침(`chart-mark-probe8.mjs`)으로 비터치 12초 뒤 svg 둥 확인: 원 두 개 모두 `cx=318`(svg 오른쪽 끝)에 고정됨 — 정상 동작. 사장님이 보낸 스크린샷(마크가 7라운드쪽에)은 ★손가락이 화면을 대고 있을 때의 정상 호버 상태★(직접 축을 이동하면 따라온다 — 요청대로). 별도 조치 안 함.
...
05:43  8회차  사이트 200 · 랭킹 중복 0 · DB 긴 질의 0 · Vercel error 0 · VPS load 2.7 · 여유 887 · 스왈 여유 687 → ★season0-apply 크론 되살림(05:44)★ · roster·hex 는 계속 끔
