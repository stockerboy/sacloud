...
>
> 마지막 갱신 **2026-09-07 13:00** · 갱신한 사람 B(실행 세션)
...
### ⚠ 09-07 에 찾은 것 — ★고치지 않고 적어만 둔다★

```
1 ★Cloud 0 창 안 경기가 0건이다★ (세 리그 전부)
  경기 목록도 「아직 경기가 없습니다」. 그래서 최근 경기 · 주간 추이 ·
  폼 TOP3 · 선수 누적(승/패/킬뎃)이 전부 빈 상태다.
  ★경기 상세(⑧)를 실제 데이터로 검증할 수 없어 그 화면에서 멈췄다.★
  (매치 상세는 542a6f7 로 시즌0 창에 걸려 있다 — 과거 경기는 주소로도 안 열린다)

2 SPL 의 현재 시즌 행만 `beta` 다 (IPL·열산은 `official`)
  같은 시각에 리그마다 시즌 이름이 갈렸다. DB 는 안 건드리고, 화면이
  `SEASON_WINDOWS` 를 보게 해서 셋 다 `Cloud 0` 으로 맞췄다.

3 IPL 클랜랭킹이 0줄 — 클랜 카드의 티어 표기 갈래를 화면으로 못 봤다 (단위 테스트로만)

4 이 노트북은 8GB 다. dev 서버 힙이 커지면 Next worker 가 죽어 페이지가 500 을 낸다.
  ★코드 문제가 아니다★ — 서버를 다시 띄우면 200 이다.
```
