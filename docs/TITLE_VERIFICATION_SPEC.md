...
## ⚠ 정정 (2026-09-01) — **고정 칭호 `[용병]` 으로 바뀌었다**

> 아래 1~11장은 **1회용 문구를 발급하는 설계**다. 사용자가 그것을 쓰지 않기로 했다.
> **원문을 지우지 않는다** (CLAUDE.md 10-4). 되돌릴 때 그대로 쓸 수 있게 남긴다.

사용자 원문 (2026-09-01):

> "회원가입은 **아디 비번 만들고 서든 계정인증만 하면 된다**
>  **칭호 [용병] 으로 바꾸면 바로 승인**되고 자기 프로필 관리 할 수 있다
>  **한줄 소개, 포지션 선택 가능**"
...
```
① 가입      아이디 + 비밀번호
② 인증      마이페이지에서 자기 서든 닉네임을 넣는다
③ 사용자가  게임에서 칭호를 [용병] 으로 바꾼다
④ 「확인」   우리가 넥슨으로 그 닉네임의 칭호를 읽는다   (닉→ouid, ouid→칭호. 2회)
⑤ [용병] 이면 바로 승인 — 그 계정이 그 사람 것이 된다
⑥ 승인되면  프로필 관리가 열린다 (한 줄 소개 · 포지션)
⑦ 칭호는 다시 바꿔도 된다
```
...
| 옛 설계 | 지금 |
|---|---|
| 발급 API (`POST` 로 문구를 받는다) | **없다.** 「확인」 하나뿐이다 |
| 칭호 풀 (2장) · `pickTitle` · `baselineTitle` | **쓰지 않는다.** 칸은 남겨 뒀다 |
| 「발급 후에 바뀌었다」 사건 판정 (1장) | **상태 판정으로 돌아갔다** — 지금 칭호가 `[용병]` 인가 |
| `no-pool` / `pool-exhausted` | **없다.** 대신 넥슨 키가 없으면 `unavailable` |

살아 있는 것: TTL(30분) · 시도 제한 · 상태 재계산 · `ouid` 기준(D-220) ·
`EggBreak` 연결과 `reason='admin'` 비덮어쓰기(6장) · 접근 통제 비우회(9장).

### ⚠ 이 방식의 알려진 약점 — **그대로 안고 간다**

고정 문구라서, **어쩌다 `[용병]` 을 달고 있는 남의 닉네임**을 다른 사람이 자기 것이라고
주장할 수 있다. 1회용 문구라면 없는 문제다. 사용자 결정대로 가되 셋으로 줄였다.

1. **먼저 인증한 사람이 임자** — `ouid` 당 `verified` 한 줄만 있게 부분 유니크 인덱스를 건다
2. **인증 시각과 그때 읽은 칭호를 남긴다** (`verifiedAt` · `lastSeenTitle`) — 다툼의 근거
3. **시도 제한** — 계정당 1시간 30회. 남의 닉네임을 계속 찔러볼 수 없다

### 구현 위치

| 무엇 | 어디 |
|---|---|
| 계약 · 순수 규칙 | `packages/contract/src/entities/titleVerification.ts` |
| 판정 · 저장 · 넥슨 호출 | `apps/web/lib/server/queries/titleVerification.ts` |
| API | `apps/web/app/api/me/title-verification/route.ts` (GET · POST · DELETE) |
| 화면 | `apps/web/app/me/link/TitleVerify.tsx` (`/me/link` 안) |
| 소유 관문이 묻는 함수 | `isTitleVerified(userId, playerId)` — 위 queries 파일 |
| 마이그레이션 (**미적용**) | `packages/db/prisma/migrations/20260901180000_title_challenge/` |

`apps/worker/src/lib/titleChallenge.ts` 와 그 테스트 32건은 **지우지 않았다.**
1회용 방식으로 되돌아갈 때 쓴다.
