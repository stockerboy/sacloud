...
> ⚠ ★**①에 함정이 있다** (2026-09-03 실측)★
> 지금 운영 `/api/health` 는 **이미 `degraded`** 다.
> ```
> status     degraded
> db         ok      쿼리 정상
...
> data       ok      공개 데이터 있음
> ```
> **`status` 한 칸만 보면 못 쓴다.** 이미 노랑이라 나빠져도 안 바뀐다.
> ★`checks.db.status` 를 봐라.★ 그게 `ok` 가 아니게 되는 순간이 진짜 신호다.
> (수집이 열흘 멈춰 있는 것 자체는 별도 문제다 — 공개 전에 사장님께 올린다.)
...
⚠ ★**아직 리허설을 안 했다**★ (`O-008` ⑧). **안 해 본 롤백은 롤백이 아니다** (최윤서).
  리허설은 **운영에 영향이 없는 방법**으로 한다 — 이전 배포 URL 을 직접 열어 200 인지 보고,
  `vercel promote --help` 가 이 PC 에서 도는지만 확인한다. ★실제로 promote 하지 마라.★
...
★깨져 있다★ 수집  마지막 성공이 240시간 전이다 (15-3 ①). 공개 전에 사장님께 올린다
```
