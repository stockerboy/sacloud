...
> ### 아직 안 된 것 (정직하게)
>
> - **진영을 아는 라운드가 10.8% 뿐이다.** 배틀로그 지표 여섯 축은 그 표본으로 낸 값이고,
>   최소치를 못 넘는 클랜은 전부 `측정중` 이다. 표본이 얇다는 사실을 화면에 병기한다
...
> | ⑤ | **플레이스타일 바 계산** (사양 8절) | 선결 둘(라운드별 진영 D-184 · 라운드 복원 D-194)은 풀렸다. 남은 것은 **구역 판정과 계산** |
