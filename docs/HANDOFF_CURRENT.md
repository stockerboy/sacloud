...
Phase 9 래더 조사 결과는 **H장**과 `docs/LADDER_PROTOTYPE_REPORT.md`에 있다.
...
## H. Phase 9 준비 — 래더 공식 조사 (2026-08-22)

> **공식은 확정되지 않았다.** production에 아무것도 적용하지 않았다.
> 전체 내용: `docs/LADDER_PROTOTYPE_REPORT.md`

### 어디까지 했나

- 프로토타입 `apps/worker/src/lab/` — **순수 함수**. 운영 코드가 import하지 않는다
- 합성 데이터 시뮬레이터 (seed 고정, DB 미사용) + 악용 시나리오 8종
- 회귀 테스트 33건 (`ladderLab.test.ts`)
- 운영 DB의 점수 필드에 **아무것도 쓰지 않았다.** 스키마도 그대로다

### 확인된 것

스펙 §1·§2 구조로 계산하면 §8의 관측 앵커가 **전부 재현된다.**
`div1 동급 패 -12 · div2 동급 패 -15 · div1이 div2에 패 -7 · div1 승리에 +11·+19 없음`

### 남은 선택 (사용자 결정)

| 항목 | 상태 |
|---|---|
| `0.6` 적용 위치 | P-C 탈락. **P-A / P-B는 관측으로 구분 불가** (교차 division 승리에서만 15%가 1점 차이) |
| 클랜 래더 | C-1 team-elo 권장. C-2(개인 증감 평균)는 **라인업 조작에 노출** |
| 클랜 상수 | 관측 분포 폭(987~1,840)이 개인과 달라 **전용 설정이 필요하다.** 값은 `[미확인]` |
| 양학 | **막히지 않는다.** D=3400이라 800점 차에서도 점수가 계속 붙는다 |
| div1 인플레이션 | 동급 경기당 +5가 주입된다 (승 +17 / 패 -12). div2는 제로섬 |
| 장기 미접속 | 감쇠 규칙이 없다 — 점수가 그대로 남는다 |

### 야간 작업 결과 (2026-08-22 08:00 종료)

| | |
|---|---|
| Phase 8.2 | ✅ 완료 · commit `d1f0d28` |
| Phase 9 준비 | ✅ 조사·시뮬레이션·프로토타입 완료 · commit `0f8618f`. **공식 미확정** |
| 검증 | typecheck · lint · build(37p) · test **365 passed / 9 skipped** · 스모크 79 · nexon:check 12 · db:check 23 |
| production 적용 | **없음.** 래더 값을 DB·화면 어디에도 쓰지 않았다 |
| 넥슨 실호출 | **0회** (누계 22회 그대로) |

**BLOCKER 아님. 다음 지시를 기다리는 상태다.**
사용자가 결정할 항목은 위 표와 `docs/LADDER_PROTOTYPE_REPORT.md` 6장에 있다.
