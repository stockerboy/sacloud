...
## H. Phase 9 — 래더 엔진 (2026-08-22 · audit 반영 완료)

> 정책: `docs/DECISIONS.md` **D-057 ~ D-078** (audit 정정 포함)
...
- **division을 넣지 않는다** (D-059) · **동급 경기 증감 합 = 0** (D-060)
- 보상 감쇠는 **부호**를 본다 (D-069) — 강자의 승리만 깎는다
  `2600이 1200을 이김 +0` / `1200이 2600을 이김 +30`
  격차 600까지는 깎지 않는다. 600~1200 선형 감소, 1200 이상 0
- **반복 대전 감쇠는 기본값 꺼짐** (D-070). 멸망전은 정상 문화다
- 시즌 전환: **전원 1500에서 시작** (soft reset 폐기 — D-064 개정)

### 공식전 인정과 용병 (D-071 ~ D-076)

**양 팀 본클랜원이 각각 3명 이상** 확인되면 공식전이다. **용병은 이 3명을 못 채운다.**

```
본클랜원 3 + 용병 2  vs  본클랜원 4 + 용병 1  → 인정 (확인 수준 5v5)
본클랜원 2 + 용병 3  vs  본클랜원 5          → 불인정
```

인정된 뒤에는 **출전이 확인된 전원**이 개인 기록·개인 래더를 받는다. 용병도 받는다.
**용병의 원소속 클랜 래더는 변하지 않는다.**

팀 배정은 추측하지 않는다 — 본클랜원 다수 결과로 두 팀의 승패를 정하고,
나머지는 **자기 승패와 같은 팀**으로 배정한다(승자가 하나뿐이므로 승패가 곧 팀이다).

확인 수준(`5v5`)은 **출전자 전원** 기준이고, 본클랜원·용병 수는 따로 저장한다.

### 시즌 (D-077 · D-078)

- 활성 시즌은 **Season 7**이다. 운영자가 `nexon:season-start`를 부르기 전까지 그대로다
- 날짜·배포로 **자동 전환하지 않는다**
- 경기의 시즌 귀속은 **실제 경기 시각(`startAt`)** 으로 정한다. 늦게 수집돼도 마찬가지다
- 새 시즌은 전원 1500에서 시작하고, **지난 시즌 기록은 건드리지 않는다**

### 실행

```bash
pnpm nexon:rate --league <slug> [--season N]   # 시즌 전체를 처음부터 다시 계산 (결정적)
pnpm nexon:season-start --league <slug>        # 새 시즌 시작 (운영자 액션)
```

### 지금 상태

**계산된 경기는 0건이다.** 로스터가 비어 재구성된 경기가 없다.
파이프라인은 오프라인 스모크 108항목으로 끝까지 검증했다(용병 기록·원소속 불변·시즌 baseline 포함).

### PRE-PRODUCTION REQUIRED CHECK

실제 roster가 등록되면 **경기 1건 이상으로 전 구간을 한 번 확인해야 한다.**
`넥슨 → 관측 → 재구성 → 인정된 Match → 래더 → LeaguePlayer/LeagueClan → 랭킹 화면`
...
- [ ] 실데이터로 `clanK` 재조정 여부 (관측 클랜 폭보다 넓게 나온다)
...
| Phase 9 조사 | ✅ `0f8618f` |
| Phase 9 구현 | ✅ `80f0dc5` |
| Phase 9 audit | ✅ 라우팅 버그 수정 · cap/용병/시즌 정책 반영 |
| 검증 | typecheck · lint · build(37p) · test **432 passed / 1 skipped** · 스모크 108 · nexon:check 15 · db:check 23 · **compare 25/25** |
| 실제 반영된 래더 | **0건** (로스터 미등록) |
