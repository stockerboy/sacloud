...
**작성 2026-08-21. 최종 갱신 2026-08-22 (Phase 9 — 래더 엔진 구현 완료).** 새 세션은 **이 파일 하나만 읽어도** 상황을 파악할 수 있어야 한다.
...
| **9 레이팅/시즌/랭킹** | ✅ 엔진·도메인 반영·UI 완료 (2026-08-22). 상수는 시뮬레이션으로 확정 |
| 10 SSR/SEO/성능/운영 | ⬜ |
...
## H. Phase 9 — 래더 엔진 (2026-08-22 구현 완료)

> 정책: `docs/DECISIONS.md` **D-057 ~ D-068** (사용자 확정)
> 상수 근거: `docs/LADDER_TUNING_REPORT.md` · 엔진: `packages/rating/`
> 조사 단계 기록(P-A/P-B 비교 등): `docs/LADDER_PROTOTYPE_REPORT.md`

### 공식

```
개인   K(R) = max(8, 36.6 - R/200)          래더가 높을수록 적게 움직인다
       E    = 1 / (1 + 10^((Ro - R) / 800))
       승리 = +round(K(R) × (1-E) × capFactor × repeatFactor)
       패배 = -round(K(R) × E × repeatFactor)

클랜   같은 구조, K = 16 고정 (Team Elo)
```

- **division을 넣지 않는다** (D-059). 2부 클랜이 1부 최하위보다 높을 수 있다
- **동급 경기 증감 합 = 0** (D-060). 원본의 1부 인플레이션(경기당 +5)을 없앴다
- `D`는 원본 3400이 아니라 **800**이다. 3400은 양학이 끝없이 이득이 된다
- 양학 억제: 점수차 300~900 구간에서 승리 보상이 선형 감소, 900 이상은 0 (D-062)
- 반복 대전: 같은 상대에게 같은 결과가 반복되면 `0.6ⁿ` (하한 0.15, 14일) (D-063)
- 시즌: `1500 + (old-1500) × 0.5` soft reset. 순위는 보존된다 (D-064)

### 경기 인정 기준이 바뀌었다 (D-057)

**양측 클랜이 각각 3명 이상** 확인되면 인정한다. `5명 2명`은 인정하지 않는다.
확인 = 그 선수의 자기 기록(목록 관측 **또는** 상세 본인 행) + 신원 확정 + 경기 시점 로스터 등록.
**로스터 등록만으로는 확인이 아니다** — 로스터는 "확인해야 할 후보" 목록이다 (D-068).

확인 수준은 숨기지 않는다. `5v4` 같은 문자열과 근거별 인원이 스테이징 → 운영 매치 → 화면
(`확인 5v4` 배지)까지 그대로 간다. 확인되지 않은 선수의 **개인 점수는 만들지 않는다** (D-067).

### 도메인 반영

`pnpm nexon:rate --league <slug>` — 리그의 재구성 경기를 `startAt` 순으로 **처음부터 다시**
계산한다. 증분 반영을 하지 않는 이유: 넥슨 수집은 도착 순서를 보장하지 않아서, 증분으로 짜면
같은 데이터에서 다른 결과가 나온다.

- 무기별 분리는 하지 않는다. 넥슨이 무기를 주지 않아 전량 `baseRating`에 누적한다 (D-034)
- mock 시드가 있는 리그는 건드리지 않는다
- 과거 3rd.supply 기록은 재계산하지 않는다 (스펙 §9)

### 지금 상태

**계산된 경기는 0건이다.** 로스터가 비어 있어 재구성된 경기가 없기 때문이다.
파이프라인은 오프라인 스모크로 끝까지 검증했다(3v3 인정 → 확인 수준 저장 → 래더 반영 →
결정적 replay → soft reset).

### 아직 사용자 결정이 필요한 것

- [ ] **로스터 등록 착수** — 이것 없이는 재구성도 래더도 0건이다
- [ ] 시즌 승강 규칙 실행 시점 (기본안: 시즌 종료 후 1부 최하위 ↔ 2부 최상위)
- [ ] 클랜 비활동 패널티 방식 (랭킹 노출 제한 / 최소 경기 수 / 점수 삭감 중)
- [ ] 관측된 클랜 래더 폭(약 850)보다 넓게 나온다 — 실데이터로 `clanK` 재조정 여부

### 작업 결과 (2026-08-22)

| | |
|---|---|
| Phase 8.2 | ✅ commit `d1f0d28` |
| Phase 9 조사 | ✅ commit `0f8618f` (P-A/P-B 비교 · 취약점 발견) |
| **Phase 9 구현** | ✅ commit `80f0dc5` — 정책 D-057~D-068 기준 |
| 검증 | typecheck · lint · build(37p) · test **408 passed / 1 skipped** · 스모크 98 · nexon:check 15 · db:check 23 · compare 24/25 |
| 실제 반영된 래더 | **0건** — 로스터가 비어 재구성 경기가 없다 (파이프라인은 스모크로 검증) |
| 넥슨 실호출 | **0회** (누계 22회 그대로) |

**BLOCKER 아님.** 다음 한 걸음은 **로스터 등록**이다. 그게 없으면 재구성도 래더도 0건이다.
