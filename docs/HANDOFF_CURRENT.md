...
> ## ⬛ 레이팅 설계 **최종 확정** (2026-08-24 · D-142)
>
> **최종 사양: `docs/RATING_FINAL_SPEC.md`** ← 다음 작업(운영 이식)의 source of truth
> 판정 근거: `docs/RATING_DESIGN_VERDICT.md` · 판정 **PASS 93/100**
>
...
```
개인  내부 = Elo(K=50, 기준 3000, 제로섬) · 단 승리 && 기대승률 >= 0.90 이면 delta=0
      표시 = 3000 + (내부 - 3000) x 신뢰도 x 3.5 - 활동 페널티
클랜  점수 = Clan Elo(K=50, 기준 3000) + 구성 보정(최근 20경기 · 상한 +50) - 활동 페널티
```
...
| 개인 K / 클랜 K | 50 / 50 |
| 기준점 | 3000 |
| 퍼포먼스(KD·MVP) | **0%** — 점수에 넣지 않는다 |
| 약팀 사냥 차단선 | 기대승률 **0.90** 이상이면 승리 delta=0 |
| 표시 배율 | **3.5** (선형 · percentile 앵커 없음) |
| 신뢰도 | 40/55/70/85/95/100% · 150판에서 100% · 이후 판수 보너스 없음 |
| 구성 보정 | 상한 **+50** · 창 **최근 20경기** |
| 개인 감점 | 4000 미만 없음 / 4000+ 주-20 / 4300+ -30 / 4600+ -40 / 4800+ -50 / 4900+ -60 |
| 감점 회복 | 경기당 -8 (개인) · -5 (클랜) — **1판으로 초기화 불가** |
| 클랜 감점 | 7~13일 -10 / 14~20일 -20 / 21일+ -30 (주당) |
| official 게이트 | **폐기.** 정상 5v5 는 전부 레이팅 대상 |

### 검증 결과

- 9시즌(150/220/500명 x 시드 3개) 평균 실력 재현도 **0.906** · FAIL 이상징후 **0건**
- 순위 결정요인: 일정 감안 승리의 질 **0.987** > 승률 0.864 > 상대강도 0.471 > 판수 **0.017**
- KD 인과 기여 **0.000** (퍼포먼스가 0이라 구조적으로 불가능)
- 고점 희귀성: 4800+ **1/9시즌** · 4900+ **0/9** · 5000+ **0/9** · 합성 outlier **5,638**
- 양학 상한: 완전 양학(600판 전승)도 **4,945** 에서 멈춘다
- 반복 대진: 멸망전 20/50/100연전 모두 점수 총합 증가 **0.00**
