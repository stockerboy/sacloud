...
> **먼저 읽을 세 파일 (2026-09-02 밤 · 기록관)**
> - 최신 인계는 `docs/HANDOFF_2026-09-02_EVENING.md`
> - 팀 규칙은 `docs/TEAM_PIPELINE.md`
> - 사장님 요청 대장은 `docs/BOSS_REQUESTS.md` (D-265 ⑤ — 총괄이 답할 때마다 통째로 보낸다)

> ## 🟦 2026-09-02 저녁 — **팀 체제로 바뀌었다. 읽는 순서가 또 바뀌었다**
>
> 사장님이 파이프라인을 정했다: **사장님 → 총괄팀 → 제작팀**.
> 아래 세 문서가 인계의 정본이다. 이 파일의 나머지 절들은 그 이전 기록이다.
>
> ```
> 1) docs/PROJECT_BRIEF.md              ← 총괄팀(매니저)용. 판 전체 · 위험 · 결정 대기
> 2) docs/HANDOFF_2026-09-02_EVENING.md ← 기술 상세 · 오늘 밟은 지뢰 목록
> 3) docs/TEAM_PIPELINE.md              ← 누가 무엇을 하고 무엇을 하면 안 되는가
> ```
>
> **오늘 가장 중요한 발견**: 병영수첩이 **진짜 브라우저에서는 그냥 열린다**(200).
> 서버에서만 403 이다. 앞 세션들이 「막혔다」고 결론지은 벽이 절반만 사실이었다.
> 받는 창구를 운영에 열어 뒀다 — `POST /api/ingest/barracks`.
> IPL 킬데스 6.3% 를 채우는 길이 이것 하나다.

> ## 🟩 2026-09-02 — **읽는 순서가 바뀌었다**
>
> 새 세션은 이 파일보다 먼저 다음 둘을 읽는다. 이 파일의 아래 절들은 **그 이전** 기록이다.
>
> ```
> 1) docs/HANDOFF_2026-09-02.md        ← 가장 최신. 밤 문서의 정정 3건이 여기 있다
> 2) docs/HANDOFF_2026-09-01_NIGHT.md  ← 그 다음. 단, 4장 첫 줄과 4-1 은 이미 틀렸다
> ```
>
> 2026-09-02 아침 기준으로 확인된 것: **래더 전수 재계산은 운영에 반영됐다**
> (supply 1,431명 · sanply 2,356명 · 최저 점수가 계획 파일과 자릿수까지 일치).
> 밀려 있던 운영 마이그레이션 3개도 적용됐다 (**D-261** — 5432 세션 풀러로 돌려야 한다).

> ## 🟥 2026-09-01 — **사이트가 흔들렸고, 원인 셋을 찾아 고쳤다.** 여기부터 읽어라
>
> 아래 절들은 **그 이전** 기록이다. 지우지 않고 남겨 두되, **지금 사실은 이 절이다.**
>
> ### 오늘 사용자가 한 말 (이게 오늘의 방향이다)
>
> > "왜자꾸 이러는거야 만약에 사람들 많이 들어오면 터지면 어떡하려고 그래"
> > "오류 없게 좀 부탁해 사이트가 탄탄했으면 좋겠어 이렇게 흔들흔들 하는게 싫어 레디한테 안좋은 기억있어서"
>
> **걱정이 정확했다.** 사이트는 사람이 몰리면 터지는 상태였다.
>
> ### 원인 셋 — 전부 「DB 를 너무 많이 때린다」로 모인다
>
> | # | 무엇 | 상태 | 근거 |
> |---|---|---|---|
> | ① | **엣지 캐시가 한 번도 걸린 적이 없었다** | **고침** | D-240 |
> | ② | 사이트와 수집이 **같은 DB 한 대**를 다툰다 | 양보시킴 | D-240 |
> | ③ | DB 통로가 **1개**라 `Promise.all` 이 순차다 | **늘리는 길은 막힘** | D-239 |
>
> ① 이 오늘 가장 큰 발견이다. `okPublic` 이 붙이는 `s-maxage` 를 **Next 가 자기 기본값으로
> 덮어쓰고 있었다.** 캐시를 일부러 안 붙인 `/api/eggs/broken` 까지 같은 머리말이 나온 것이
> 증거다. `next.config.ts` 의 `headers()` 로 옮겨 걸었더니 **작동한다** (`X-Vercel-Cache: HIT`).
>
> ③ 은 `connection_limit` 을 1→5 로 올려 봤더니 **Supabase 풀러가 먼저 무너졌다**
> (`ECHECKOUTTIMEOUT`). 되돌렸다. **통로를 늘리는 길은 막혔다.**
>
> ### 그래서 한 것
>
> ```
> 엣지 캐시     next.config.ts headers() 로 27경로 · 등급 3단(3600/300/30)
>              stale-while-revalidate 86400 ← DB 가 죽어도 500 을 안 보는 근거
> 캐시 워밍     .github/workflows/cache-warm.yml · 5분마다 15경로를 깨운다
> 수집 양보     self-dispatch 앞에 15분 쉼 (timeout 50→70분)
> 왕복 줄이기   선수상세 39→21 · 클랜상세 25→20 · home/top 13→8   (D-243)
> 육각형        리그 전체 7MB 읽던 것을 요약표 164KB 로 (D-238 · D-241)
> ```
>
> ### 부하 시험 실측 (`node scripts/load-test.mjs`)
>
> ```
> 엣지가 받아 낼 때   동시 5명 오류 0% · 동시 10명 오류 0.5% · 보통 33~37ms
> 전부 DB 까지 갈 때   동시 5명이 한계 · 동시 10명에서 12.3% 실패
> ```
>
> **지금 사이트를 지탱하는 건 캐시다.** 캐시가 비면 5명에서 무너진다.
> 그래서 워밍이 중요하고, 오픈 전에 **동시 30·50 으로 다시 재야 한다.**
>
> ### 아직 안 한 것 · 사용자 결정이 필요한 것
>
> - **DB 를 키우거나 나눈다** — Supabase 상위 플랜 또는 읽기 전용 사본. **돈이 드는 결정**이다
> - `getFormTop` 의 시간대 함정 (D-243 이 같은 버그를 다른 곳에서 고쳤다). **값이 바뀌므로 판단 필요**
> - 15분 · 300초 · 86400초는 전부 **실측으로 다시 정해야 하는 임시값**
>
> ### 오늘 배운 것 — **같은 실수를 세 번 했다**
>
> | | 잰 줄 알았던 것 | 실제로 잰 것 |
> |---|---|---|
> | D-223 | 클랜 상세 1.2초 | **엣지 캐시 HIT** |
> | D-238 | 이 질의는 585ms | **로컬 직결**에서만 |
> | D-239 | 통로를 늘리면 빨라진다 | **풀러 용량을 안 봤다** |
>
> **성능은 「그 값이 나올 자리」에서만 잰다.** 로컬 숫자는 «여기서는 이랬다» 이지
> «운영에서도 이럴 것이다» 가 아니다.
>
> ### 오늘 들어온 기능 (성능 외)
>
> ```
> 클랜 육각형 V2  화면까지 연결 · 운영에 9,388행 + 요약 155행 (D-235 · D-241)
...
> ### ⚠ 이름이 바뀐다 — **아직 바꾸지 마라**
>
> `SACLOUD` → **`3rd cloud`** (D-242). **시안이 확정된 뒤 한 번에** 바꾼다.
> 바꾸는 것은 **사람이 읽는 글자 10곳뿐**이고 `@sacloud/*` 패키지 이름은 안 바꾼다.


> ## ⬛ 세션재개3 — **사용자 사양(SITE_SPEC_V2) 구현분** (2026-08-30 오후)
>
> 아래 절들은 **그 이전** 기록이다. 지우지 않고 남겨 두되, **지금 사실은 이 절이다.**
>
> ### 사용자가 준 사양
>
> `docs/SITE_SPEC_V2.md` 에 **원문 그대로** 박아 두었다. 해석과 구현 상태는 그 아래에 있다.
> 지시와 함께 온 말: *"오늘 오후 10시 전에 (…) 사이트 완성시켜줘. 나한테 이제 더 이상
> 묻지 말고 혼자 자율로 진행해. 내가 정말 필요할때는 그 부분은 빼고(ex 등록관련)"*
>
> ### 무엇이 들어왔나
>
> | 커밋 | 무엇 |
> |---|---|
> | `38853d1` | 사양 5종 구현 — 개인 티어별+천적 · 클랜 DB 지표 · 메인 · 게시판 반익명 · 라운드 지표 재료. 교차검증이 잡은 결함 4건 동시 수정 |
> | `a9d2814` | D-195 ~ D-200 문서화 + SPL·IPL 게시판 |
> | `a18c5de` | 육각형을 촘촘하게(눈금 5겹 · 점 2.1 · 너비 상한) + 수집 파일 형식 두 가지 수용 |
> | `e1d185f` | 포지션별 클랜원 명단(1군/2군) · 경기상세 포지션+`(S)` · 배틀로그 라운드 지표 6축 |
> | `af6a4a2` | 같은 하루가 개인페이지에 두 번 나오던 것 |
> | `59cedb7` | 수집 목록을 시즌0 창 + 포지션 미상 우선으로 |
>
...
> ClanRoundProfile      클랜 라운드 집계        109팀 · 진영 아는 라운드 90.6%  (v3 · D-208)
> PlayerPlaystyleProfile 플레이스타일 집계      4,169명 · 선수 연결 3,521  (v1 · D-211)
...
> ### 아직 안 된 것 (정직하게)
>
> - ~~진영을 아는 라운드가 10.8% 뿐이다~~ → **90.6% 다** (D-208). 클랜 육각형 다섯 축이
>   실제로 찼다 — 평균 측정 축수 3.01 → 4.03 / 6 (D-211).
>   ⚠ 같은 작업에서 **최소 표본을 20 → 100 라운드**로 올렸다. 20 그대로였다면 5.34/6 이지만,
>   그 표본에서는 표시 오차(±0.54라운드)가 화면 자릿수(0.1)보다 다섯 배 컸다
> - **블루방어율·조직력·폭발력은 100라운드에서도 줄 세우기가 약하다** (신뢰도 0.16~0.21).
>   클랜 사이의 진짜 차이가 2%p 안팎이라 표본을 더 쌓아도 크게 안 나아진다.
>   백분위로 보여 주는 것이 옳은지는 **사용자가 정할 문제다** (D-211)
> - **포지션 판정 1,031명 / 전체 4,643명.** 나머지는 `포지션 미정` 이다. 지어내지 않는다
> - **회원가입 · 본인인증 · 계정 소유권 증명은 범위 밖이다** (사용자가 빼라고 했다)
> - 화력의 자릿수 기준이 확정되지 않았다 — 팀 합계와 1인 환산을 **둘 다** 낸다 `[미확인]`
>

> ## ⬛ 세션재개3 — **밤샘분 최종 정리** (2026-08-30 새벽 · D-185 ~ D-187 · D-194)
>
> 아래 "세션재개3 (앞부분)" 은 어젯밤 **초반**에 쓴 것이고 이미 낡은 서술이 섞여 있다.
> 지우지 않고 남겨 두되, **지금 사실은 이 절이다.**
>
> ### 어젯밤에 실제로 끝난 것 — 커밋 6개
>
> | 커밋 | 무엇 |
> |---|---|
> | `ce99d46` | 전투력 육각형 · 플레이스타일 바 · 오늘 기록 (D-185 · D-186). 모르는 축을 0으로 그리지 않는다. `최근 폼`(6개월 꺾은선)을 빼고 그 자리에 오늘 기록. **하루 경계 오전 7시 KST · 무경기 문구 `미접속`** |
> | `e9b1f2a` | **5433 간헐 실패의 진짜 원인 규명** (D-187). 저장소가 아니라 **이 컴퓨터의 소켓 계층**이다 — 생 TCP 로도 `EFAULT` 가 나고 **닫힌 포트에서도** 난다. 로컬 루프백에 한해 `connection_limit=5` |
> | `4959efa` | **라운드 복원 모듈** (D-194). `packages/nexon/src/roundState.ts` (순수 함수). 이벤트 한 줄이 죽은 사람 한 명을 가리킨다 → 라운드별 생존자 복원 |
> | `5b423ea` | **P1001 재시도**로 5433 실패를 실질적으로 끝냈다 (로컬 루프백만 · 최대 8회) + `PlayerRoundProfile` 표 (D-187 · D-194). 조용히 skip 되던 테스트 70여 건이 드러났다 (skip 104 → 32) |
> | `fd3169a` | **라운드 복원을 육각형에 물렸다** (D-194). `nexon round-build` 신설. **라플수 5/6 · 스나수 3/6** 축이 찼다 |
> | `0555a27` | 경기 길이를 배틀로그에서 직접 잰다 (D-194). `Match` 조인 대신 마지막 이벤트 시각의 **하한**. 매치의사나이 확정 26 → 209 |
>
> ### 지금 육각형이 어디까지 차나 (`packages/contract/src/traits.ts` 실측)
>
> ```
> 라플수  5/6   세이브 · 샷싸움 · 캐리력 · 매치의사나이 · 소수싸움
>               남은 것: 원어택 성공률  → 포지션 판정 선행
> 스나수  3/6   세이브 · 캐리력 · 소수싸움
...
> 표본이 늘면 자연히 5/6 이 된다.
>               남은 것: 스나싸움 · 작업 성공률  → 킬로그의 상대 무기 판정 필요
> ```
>
> ### 로컬 DB 실측 (2026-08-30)
>
> ```
> BarracksBattleLogRaw  subjectKind='clan'   2,760행 · 고유 경기 2,449건   ← 새로 들어왔다
> BarracksBattleLogRaw  subjectKind='user'     786행                      ← 어제와 같다
> 라운드 복원 성공                              2,440 / 2,449 (99.6%) · 계정 불명 0
> PlayerRoundProfile                          2,696명 · 우리 Player 연결 1,975명
> PlayerPositionProfile                          25명                     ← 어제와 같다
> ```
>
> ### ⚠ 아직 화면을 **눈으로 못 봤다**
>
> 이 컴퓨터에서 `next dev` 의 `listen()` 이 `EFAULT` 로 죽어 **개발 서버가 아예 뜨지 않는다**
> (D-187). 순수 `http.createServer` 도 같다. 원인은 저장소가 아니라 컴퓨터의 소켓 계층이고,
> 고칠 곳도 컴퓨터다 (`netsh winsock reset` + 재부팅 · 네트워크 필터 드라이버 점검).
>
> **사용자가 `pnpm dev:clean` 으로 서버를 띄워 줘야 검수가 된다.**
> 그전까지 육각형 · 플레이스타일 바 · 오늘 기록 · 옮긴 `상세정보` 는
> **"구현했다" 이지 "검수했다" 가 아니다** (`CLAUDE.md` 3장 10번).
>
> ### ⚠ 운영에 **push 하지 않았다**
>
> ```
> git log origin/main..main --oneline | wc -l   →  12   (2026-08-30 측정)
> origin/main                                   →  52033b5 (D-181 · IPL 등록 클랜 39곳)
> ```
>
> 로컬 커밋 **12개**가 밀려 있다. 어젯밤 6개 + 그 앞의 5개 + `5b8a94d`.
> 운영은 여전히 `52033b5` 다. 즉 **육각형도 라운드 복원도 운영 사이트에는 없다.**
>
> ⚠ 이 숫자는 커밋이 늘 때마다 바뀐다. **읽는 사람은 위 명령을 직접 다시 돌려 확인해라.**
>
> ### ⚠ 마이그레이션 드리프트 — **운영 배포 전에 반드시 정리한다**
>
> ```
> _prisma_migrations 실측 (2026-08-30)
>   20260823120000_public_data_origin   finished_at = null · rolled_back_at = 2026-08-23
>   → 같은 이름의 디렉터리가 packages/db/prisma/migrations 에 **없다**
> ```
>
> 이 줄 때문에 `prisma migrate dev` 가 **DB 리셋을 요구한다.** 36만 경기가 날아간다.
> 그래서 `PlayerRoundProfile`(`20260830020000_player_round_profile`)은
> `db execute` 로 SQL 을 직접 넣고 `migrate resolve --applied` 로 이력에 표시했다.
> **드리프트는 그대로 남아 있다.** 운영 배포 전에 정리해야 한다.
>
> ### 다음 사람이 할 일 — 우선순위
>
> | # | 할 일 | 막고 있는 것 |
> |---|---|---|
> | ① | **화면 검수** — `/league/supply/player/<id>` 를 열어 육각형·바·오늘 기록·옮긴 `상세정보`·모바일 순서를 눈으로 본다 | 사용자가 dev 서버를 띄워 줘야 한다 (D-187) |
> | ② | **원어택 성공률** (육각형 5번 · 라플) | **포지션 판정 선행.** `PlayerPositionProfile` 이 25명뿐이다 |
> | ③ | **스나싸움 · 작업 성공률** (육각형 2·5번 · 스나) | 킬로그의 **상대가 그 경기에서 든 무기** 판정 |
> | ④ | **우리 MVP 저장·표시** (사양 6절) | 재료(`lastRoundTopKiller`)는 있다. **경기별로 담을 칸이 없고 화면에 안 물렸다** |
> | ⑤ | ~~플레이스타일 바 계산~~ → **됐다** (D-211) | 블루 `자리 흩어짐`(0.877) · 레드 `첫 교전 지연`(0.697). 실측 **블루 3,475명 · 레드 3,493명**에게 뜬다. 사양의 **구역 이름 정의는 못 썼다** — 좌표가 확정된 적이 없어 대체 재료를 골랐다 `[미확인]` |
> | ⑤-2 | 플레이스타일 잡을 `nexon` 하위 명령으로 옮기기 | 지금은 `pnpm --filter @sacloud/worker playstyle-build`. `cli.ts` 가 동시 작업 중이라 안 건드렸다 — **한 줄이면 된다** |
> | ⑥ | **운영 DB 반영 규명** | D-191 — 로컬↔운영 대조 결과 반영은 **보류** 상태다 |
> | ⑦ | **IPL 이관** | D-181 |
>
> 검수 항목의 상세 목록은 아래 "세션재개3 (앞부분)" 의 `다음 사람이 할 일` 1번에 있다.
> 그 목록은 여전히 유효하다.

> ## ⬛ 세션재개3 (앞부분) — 전투력 육각형 · 플레이스타일 바 · 카드 배치 (D-185) · **렌더 검수 미완** (2026-08-29)
>
> ⚠ **이 절은 어젯밤 초반에 쓴 것이다. 아래 서술 중 일부는 그 뒤에 뒤집혔다** —
> "아직 커밋하지 않았다"(→ `ce99d46` 등 6개로 커밋됨) · "클랜 단위 배틀로그 0건"(→ 2,760행) ·
> "스나수는 캐리력 하나뿐"(→ 3/6) · 축 표(→ 위 절 참조). **지금 사실은 위 절이다.**
>
...
>
> ### ~~지금 되는 축 / 안 되는 축~~ → **바뀌었다** (D-194 · 위 절 표를 봐라)
>
> | 축 | 라플수 | 스나수 |
> |---|---|---|
> | 3 캐리력 (판당 평균 킬) | ✅ | ✅ |
...
> → **바뀌었다.** 클랜 단위 2,760행이 들어왔고 라운드 복원도 됐다 (D-194).
>
> ### ⚠ 막힌 것 — **화면 렌더를 아직 못 봤다**
>
...
> 2. ~~**커밋** — 지금 작업 전체가 커밋되지 않은 상태다.~~ → **했다** (`ce99d46` 외 5개).
>    `apps/web/probe-traits.mts` 는 임시 점검 스크립트이므로 **커밋에서 뺐다**.
>    다만 **push 는 아직 안 했다** — 미푸시 11개
...
>    → **P1001 재시도로 실질 해소**됐다 (`5b423ea` · D-187). 원인은 훅이 아니라 소켓이었다
> 4. ~~**클랜 단위 배틀로그 수집** (D-184)~~ → **들어왔다** (2,760행). 라운드 복원까지 됐다 (D-194)
> 5. **사양 6절 MVP 규칙은 아직 미구현이다** — 손대지 않았다.
>    → **여전히 미구현이다.** 재료 함수(`lastRoundTopKiller`)만 생겼다
>
> ### 사용자 확정이 필요한 `[미확인]`
>
> | 값 | 지금 | 왜 |
