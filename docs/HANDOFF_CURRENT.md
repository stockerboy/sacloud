...
> ## ⬛ 세션재개3 — 전투력 육각형 · 플레이스타일 바 · 카드 배치 (D-185) · **렌더 검수 미완** (2026-08-29)
>
> `docs/PLAYER_TRAITS_SPEC.md` 4·8·9·10절을 화면으로 만들었다. **아직 커밋하지 않았다.**
>
> ### 시작 상태 점검
>
> | | |
> |---|---|
> | 직전 커밋 | `f96a895` (D-184 · 수집기가 원문을 localStorage 에 쌓지 않게) |
> | 로컬 DB | 살아 있다 (`127.0.0.1:5433` LISTENING · PID 8592) |
> | typecheck / lint | 통과 |
> | test | **1,342 passed · 3 failed · 55 skipped / 1,400** |
>
> 실패 3건은 `playerPosition.test.ts`(2) · `playerProfileIdentity.test.ts`(1) 이고 전부
> **병렬 실행 중 5초 타임아웃**이다. 세션재개2 가 기록한 그 간헐 실패이고 D-185 와 무관하다.
>
> ### D-185 로 만든 것
>
> | 무엇 | 어디 |
...
> | 응답 스키마 | `packages/contract/src/entities/detail.ts` (`PlayerTraits` · `PlayerPlaystyle`) |
> | 재료 집계 + 리그 분포 캐시 | `apps/web/lib/server/queries/playerTraits.ts` **(새 파일)** |
> | 응답에 물림 | `apps/web/lib/server/queries/records.ts` (`getLeaguePlayerDetail`) |
...
> | 토큰·애니메이션 | `packages/ui/src/styles.css` (`--color-side-meta` · `trait-heartbeat`) |
> | 배치 (사양 9절) | `apps/web/app/league/[leagueSlug]/player/[playerId]/page.tsx` |
> | 새 테스트 | `traits.test.ts`(45) · `trait-copy.test.ts`(43) · `playerTraits.test.ts`(9) |
>
> Mock 과 실제 서버가 **같은 `buildPlayerTraits()`** 를 부른다. 재료를 세는 방법만 다르다.
>
> ### 지금 되는 축 / 안 되는 축
>
> | 축 | 라플수 | 스나수 |
> |---|---|---|
> | 3 캐리력 (판당 평균 킬) | ✅ | ✅ |
> | 2 샷싸움(딜량) / 스나싸움 | ✅ | ❌ `배틀로그 필요` |
> | 5 원어택 성공률 / 작업 성공률 | ❌ `포지션 판정 필요` | ❌ `배틀로그 필요` |
> | 1 세이브 · 4 매치의사나이 · 6 소수싸움 | ❌ `라운드 복원 필요` | ❌ `라운드 복원 필요` |
> | 플레이스타일 바 2줄 | ❌ `배틀로그 필요` | ❌ `배틀로그 필요` |
>
> **스나수는 잴 수 있는 축이 캐리력 하나뿐이다.** 값이 없는 축은 `percentile: null` 이고
> **0으로 그리지 않는다** — 여섯 축을 다 재야 도형을 잇고, 하나도 못 재면 작은 정육각형이
> 심장처럼 뛴다. 플레이스타일 바도 가운데(`정석`)로 채우지 않는다.
>
> 막고 있는 것은 하나다 — **클랜 단위 배틀로그 수집분이 0건**(D-184). `subjectKind='user'`
> 786건은 있지만 그것으로는 라운드 복원이 안 된다.
>
> ### ⚠ 막힌 것 — **화면 렌더를 아직 못 봤다**
>
> 이 실행 환경에서 서버 소켓이 열리지 않는다.
>
> ```
> listen EFAULT: bad address in system call argument 127.0.0.1:3999   (실측)
> ```
>
> Next dev 서버든 순수 `http.createServer` 든 같다. 그래서 `CLAUDE.md` 3장 10번이 요구하는
> **실제 렌더 스크린샷 확인이 통째로 빠져 있다.** 육각형·바·오늘 줄·옮긴 `상세정보` 는
> **"구현했다" 이지 "검수했다" 가 아니다.**
>
> **사용자가 `pnpm dev:clean` 으로 서버를 띄워 줘야 한다.**
>
> ### 다음 사람이 할 일
>
> 1. **렌더 검수** — `/league/supply/player/<id>` 를 열어 확인한다.
>    - 육각형이 그려지는가 · 잰 두 축만 점으로 찍히고 **도형이 안 그려지는가**
>    - 축 이름이 주무기를 따라 바뀌는가 (`샷싸움`/`스나싸움` · `원어택`/`작업 성공률`)
>    - `측정중 N항목 — …` 요약 줄과 `같은 라플수 N명 안에서 견줬습니다` 가 맞는가
>    - `--color-side-meta`(#94a3b8) 글자가 어두운 패널 위에서 읽히는가 — **실측값이 없는
>      `[미확인]` 색이다**
>    - 심장박동 육각형이 데이터처럼 읽히지 않는가
>    - **모바일**: `상세정보` 가 위 카드로 올라갔고 `최근 같이한 플레이어` 가 경기 기록
>      **아래**에 있는가 (`order` 뒤집기를 걷어냈다 — 여기가 틀어지면 2026-08-28 지시 위반)
> 2. **커밋** — 지금 작업 전체가 커밋되지 않은 상태다. `apps/web/probe-traits.mts` 는
>    임시 점검 스크립트이므로 **커밋에서 뺀다**
> 3. **테스트 빚** — `apps/web/tests/playerTraits.test.ts` 가 전체 실행에서 `afterAll` 훅
>    10초 타임아웃으로 스위트째 실패할 때가 있다(단독은 9/9 · 7초 통과). `T185-` 픽스처
>    잔여물은 **0건으로 확인**했지만, 훅 타임아웃을 올리든 픽스처를 줄이든 손봐야 한다
> 4. **클랜 단위 배틀로그 수집** (D-184) — 이게 들어오면 육각형 네 축과 바 두 줄이 함께
>    살아난다. 사용자 브라우저에서 스니펫을 돌려야 한다
> 5. **사양 6절 MVP 규칙은 아직 미구현이다** — 손대지 않았다
>
> ### 사용자 확정이 필요한 `[미확인]`
>
> | 값 | 지금 | 왜 |
> |---|---|---|
> | `TRAIT_MIN_GAMES` | `10` | 사양 4절에 숫자가 없다. 우리가 골랐다 |
> | `DISTRIBUTION_TTL_MS` | `10분` | 사양에 없다. 백분위가 그만큼 낡는다 |
> | `--color-side-meta` | `#94a3b8` | 원본에 이 블록이 없어 실측할 값이 없다 |

> ## ⬛ 세션재개2 — 밀려 있던 D-175~D-180 을 **커밋으로 확정** (2026-08-29)
>
> 앞 세션이 작업 도중 멈췄고, 고친 파일 64개가 커밋 없이 떠 있었다. 그것부터 확정했다.
>
> | 커밋 | 무엇 |
> |---|---|
> | `d2666e7` | 시즌0 창 = `2026-04-01(KST) ~ 현재`(열린 구간) · `season0-finish` (D-175) |
> | `c83df72` | 미러 경기는 저장된 진영 클랜이 정본 (D-180) |
> | `4753b24` | 화면 성적 수치를 시즌0 창으로 통일 · `시즌0` 표기 (D-176 · D-178) |
> | `4426eb1` | 대룰리그 서비스 준비중 (D-178) |
> | `0b6633f` | **wip** — 병영수첩 미니맵 정렬 4차. **안 맞는다** (D-174) |
>
> **운영 DB 는 이번에도 손대지 않았다.** 코드만 커밋했다. `sanply`·`supply` 재계산 결과는
> 여전히 **로컬 DB 상태**이고, 운영 반영은 사용자가 판단할 일이다.
>
> ### 검증 (커밋 전)
>
> | | |
...
> | test 1차 | 1,203 passed · 2 failed |
> | test 2차 | 1,245 passed · 3 failed |
>
> **실패가 매 실행마다 자리를 옮긴다 — 회귀가 아니다.** 두 실행에서 겹치는 실패가 없다.
>
> - `health.test.ts` — 다른 테스트가 리그·경기를 만들었다 지우는 사이에 세는 교차간섭.
>   테스트 자신이 주석으로 인정한 것이고 예전부터 있었다
> - 나머지 전부 `Can't reach database server 5433`. **DB 프로세스는 안 죽었다**(PID 동일).
>   `playerPosition.test.ts` 는 단독 실행 시 1.5초에 통과한다 — **병렬일 때만** 깨진다.
>   인수인계 문서에 적힌 그 미해결 간헐 실패다
> - 1차의 `weaponRanking.test.ts` 는 앞 세션이 죽으며 남긴 픽스처 잔여물(`T166-`)이었다.
>   지웠고 이후 통과한다. `afterAll` 이 `beforeAll` 실패 뒤에도 돌아 스스로 치운다
>
> ### `.gitignore` 에 `apps/worker/reports/` 를 넣었다
>
> 3.4MB 짜리 replay 산출물이다. 이미 무시하고 있는 `apps/worker/backups/` 와 같은 성격이라
> 저장소에 넣지 않는다.
>
> ### ⚠ 이어서 할 일 — 미니맵 정렬이 **막혀 있다**
>
> `0b6633f` 는 **완료가 아니다.** 좌표의 28.9% 만 맵 안쪽에 들어간다(맞았다면 90%+).
>
> - 원인은 뒤집기가 아니다 — normal/flipX/flipY/flipXY **네 가지 다** 안 맞았다
> - 원인은 **마스크**다. 벽 선이 안 닫혀 flood fill 이 안쪽까지 먹었고,
>   "맵 안쪽" 이 그림의 **12.5%** 로만 잡혔다. 실제 교전 공간은 그보다 훨씬 넓다
> - `data/barracks/map-align.json` 에 `status: 미해결` 을 박아 뒀다. **그 값을 쓰지 마라**
> - 이게 풀려야 `docs/PLAYER_TRAITS_SPEC.md` 8절의 구역 이름
>   (`컨뒤`·`설대`·`달방`·`ㄱ자`·`벙커`·`비롱`)을 좌표로 찍을 수 있다

> ## ⬛ 화면을 시즌0 창으로 통일 · 래더 판정 = 엔진 · `시즌0` 표기 · 대룰리그 접기 — **로컬까지만** (2026-08-29 · D-178)
>
> | | |
> |---|---|
> | 커밋 | `4753b24` (수치·표기) · `4426eb1` (대룰리그 접기) — 세션재개2 에서 반영 |
> | 운영 DB | **손대지 않았다.** 읽지도 않았다 |
> | 범위 | `apps/web` · `packages/ui` · `packages/contract` · `docs` 만 |
>
> ### 무엇이 바뀌었나
>
> - **화면 래더 판정에 `origin='nexon'` 을 더했다** (D-164 를 넓혔다).
>   시즌0 replay 가 원본 `Match.redRatingUpdate` 를 채우지 않아(D-171) 넥슨 경기 38건이
>   화면에서만 빠져 있었다 — 같은 선수를 **엔진 70판 · 화면 66판**으로 셌다
> - **성적 수치를 전부 시즌0 창 기준으로 통일**(사용자 A안) — 상세정보 · 최근매치 요약 ·
>   연승연패 · 평균킬 분모 · 폼 그래프 · 폼 TOP3. **기록실·매치 상세·지난시즌 카드는 그대로다**
> - 창 값은 **화면에 복제하지 않았다.** `apps/web/lib/server/queries/season0Scope.ts` 가
>   `apps/worker/src/lib/season0Window.ts` 를 **상대 경로로 읽기만** 한다.
>   `pnpm --filter @sacloud/web typecheck` · `next build` 둘 다 통과 확인
> - 화면 표기 `Beta Season` → **`시즌0`**. 문자열은 `packages/contract/src/season.ts` 한 곳.
>   `@sacloud/db/ops` 의 `seasonLabel()` 은 CLI 용이라 그대로 뒀다
> - **대룰리그(`daerule`) 접음** — `/league/daerule/**` 전 경로가 `서비스 준비중` 안내.
>   판정은 `packages/ui/src/site-config.ts` 의 `PREPARING_LEAGUE_SLUGS` 한 배열.
>   **데이터는 그대로 있고** GNB 링크도 남겨 뒀다
>
> ### 로컬 실측 (2026-08-29)
>
> | | 전 | 후 |
...
> | `서든러#001088394` 프로필 판수 | 66 | **70** |
>
> 프로필 API = 랭킹이 읽는 `LeaguePlayer` 누적과 **전부 일치** —
> 34승 36패 48.6% · 573킬 584데스 49.5% · 평균킬 8.3 · MVP 5 · 래더 2,873 · 1,096명중 735위
>
> ### ⚠ 꼭 알아야 할 것
>
> - **엔진은 `sourceMatchId` 로 미러/넥슨 중복을 빼지만 화면은 안 뺀다.**
>   현재 한 리그 안 중복 0건이라 차이가 없다. 두 수집이 같은 리그에서 만나면 화면만 두 번 센다
> - `/api/leagues/daerule/...` 는 **여전히 응답한다.** 화면 링크만 없앴다
> - 평균킬 분모는 `knownGames`(K/D 를 아는 판수 · D-176)라 69 다. 엔진 보고서의 8.19 는
>   70 으로 나눈 값이다 — 정의 차이이고 이번 변경으로 생긴 것이 아니다

> ## ⬛ 열산리그 시즌0 재계산 · 대룰리그 보류 · `side_clan_mismatch` 규명 — **로컬까지만** (2026-08-29 · D-179)
>
> | | |
> |---|---|
> | 커밋 | `c83df72` (D-180) — 세션재개2 에서 반영. `sanply` 재계산분은 로컬 DB 상태다 |
> | 운영 DB | **읽지도 쓰지도 않았다** |
> | 로컬 DB | `sanply` 만 `season0Apply --confirm` 반영. 백업 `applied-2026-08-29T03-30-31-012Z.json` |
>
> ### `sanply`(열산리그) — `supply` 와 같은 창·같은 공식
>
> - 창 안 **24,995** (미러 24,995 · 넥슨 0 · 중복 0) · 클랜 래더 반영 **22,318** ·
>   개인 증감 계산 **24,989**
> - 집계 선수 **4,315명** · 클랜 **120곳** · 랭킹 노출 **2,266명** · 배치고사 2,049명
> - 시즌0 무경기라 기준점으로 되돌린 선수 10,997 · 클랜 236
> - 자기 점검 — 무기분리 불일치 0 · 무기 미상 0 · NaN 0 · 불변식 어긋난 선수 **0**
> - TOP5 — `pit` 5189 · `피존` 5011 · `글썽` 4695 · `코딱지파는중` 4607 · `1rock` 4601
> - 제로섬 오차 10,966.5 → D-175 와 같은 성격의 **`[미확인]`**
>
> ### `daerule`(대룰리그) — **계산하지 않는다**
>
> 사용자 지시 · **서비스 준비중**이라서다. **데이터는 그대로 있다** (창 안 1,591 · 창 밖 28,106).
> `season0.ts` 기본 목록에서 빼지 않았다 — `--leagues` 로 지정해서 쓴다.
> 화면 안내 문구는 다른 담당이다.
>
> ### `side_clan_mismatch` — 원인 규명 완료 · **고치지 않았다**
>
> 전체 조사: **`docs/SIDE_CLAN_MISMATCH_AUDIT.md`**
>
> - 원인은 기록이 아니라 **판정 우선순위**다. 참가자 원소속 다수결이 저장된 진영 클랜을 덮는다.
>   그 다수결의 입력은 **41%가 비어 있고**, 어긋난 쪽의 **절반은 단 한 명**이 팀 이름을 정한다
> - ⚠ **개인 래더는 빠지지 않는다. 클랜 래더만 빠진다.** 검사가 개인 반영 **뒤**에 있어서다.
>   그래서 `LeagueClan` 은 승·패에는 세어졌는데 판수에는 안 세어진 경기를 갖고 있다
>   (`supply` 47곳 중 **43곳** · `sanply` 120곳 중 **110곳**)
> - 권고 1(미러는 기록이 정본) → 어긋남 1,390 → **0** · 권고 2(다수결 3명 이상) → **56**.
>   **규칙 변경은 사용자 승인 사항**이라 코드는 한 줄도 안 바꿨다

> ## ⬛ 시즌0 창 재정의 · 마감 명령 — **로컬까지만** (2026-08-29 · D-175)
>
> | | |
> |---|---|
> | 커밋 | `d2666e7` — 세션재개2 에서 반영 |
> | 운영 DB | **손대지 않았다.** 읽기도 하지 않았다. 반영은 사용자가 한다 |
> | 로컬 DB | `season0` replay → `season0Apply --confirm` 반영 완료 |
>
> ### 무엇이 바뀌었나
>
> - **시즌0 창 = `2026-04-01 00:00 KST` ~ 현재(열린 구간).** 끝을 박지 않는다.
>   예전 `2026-01-01 ~ 2026-07-01` 고정값 때문에 **7월 이후 경기가 시즌0 집계에서 통째로 빠졌다**
> - 창·origin·시즌 번호는 이제 **`apps/worker/src/lib/season0Window.ts` 한 곳에만** 있다
> - 대상 origin 이 미러 + **넥슨** 둘 다가 됐다. 중복은 `sourceMatchId` 로 제거(미러 우선)
> - D-170 미러 가드는 **`matchScope` replay 에는 걸지 않는다.** 평소 계산에는 그대로 걸린다
> - `nexon season0-finish` 신설 — 시즌0 마감 + 지난시즌 카드. `--confirm` 없이는 한 줄도 안 쓴다
> - `closeSeason` 이 만 명 리그에서 **트랜잭션 시간 초과로 죽고 있었다** → `createMany` 로 교체
> - `SEASON_BASELINE` 이 1500 으로 남아 있었다 (D-145 는 3000) → 3000 으로 맞춤
>
> ### 로컬 실측
>
> - 창 안 경기 **10,019** (미러 9,883 + 넥슨 136 · 중복 0) · 래더 반영 8,609
> - 집계 선수 **2,382명** · 랭킹 노출(배치고사 아님) **1,096명** · 클랜 47곳
> - 창 밖(2024-05 ~ 2026-03) **120,139건 그대로 보존** — 기록실은 창으로 거르지 않는다
> - 문제 선수 `서든러#001088394` — 0승 0패 · `배치고사` → **70전 34승 36패 · 평균킬 8.19 · MVP 5 · 2,873점**
>
> ### ⚠ 꼭 알아야 할 것
>
> - **"시즌1" 의 번호가 `[미확인]`이다.** 이관 시즌 1~7 이 이미 있어 다음 번호는 8 이다.
>   그래서 `season0-finish --open` 은 `--number N` 을 **반드시** 받는다
> - 마감 명령은 로컬에서 한 번 돌려 카드 2,382장을 확인한 뒤 **되돌렸다.** 시즌0 은 여전히 열려 있다
> - ~~`sanply` · `daerule` 리그는 아직 재계산하지 않았다~~ → **D-179**:
>   `sanply` 는 로컬 반영 완료 · `daerule` 은 사용자 지시로 접었다(데이터는 보존)

> ## ⬛ 시즌0 우리 공식(v2) 반영 · 미참여 감점 정상화 — **운영까지 끝났다** (2026-08-29 · D-170~D-173)
>
> | | |
> |---|---|
> | 커밋 | `8e7ea42` — `origin/main` 반영 확인 |
> | 운영 DB | **반영됨** (`season0ApplyProd`) · 백업 `apps/worker/backups/season0/applied-*.json` |
> | 운영 화면 | `/api/leagues/supply/ranks/players?weapon=rifle` 1위가 로컬 replay 결과와 **일치** |
>
> ### 무엇이 바뀌었나
>
> - **점수 시스템 v2** (D-172) — 팀 대 팀 기대승률 · 억제/신뢰도/승률자격선 제거 ·
>   판수 구간별 K · 본클랜원 가중치는 **클랜 점수에만**. 실데이터 14,408경기 백테스트로 전 항목 우세
> - **미참여 감점이 한 번도 작동한 적 없던 버그**를 고쳤다 (D-173) — 감점 커서를 매 경기 시각으로
>   밀어 "하루가 지났는가" 가 영원히 거짓이었다. 7일 유예 후 **30일이면 기준점(3000)**
> - **무기 랭킹은 주무기만** (`LeaguePlayerWeaponStat.isMain`) · 감점을 **무기 증감에도 같은 비율로**
> - 미러 구간은 **원본 점수가 정본**이다 (D-170). 시즌0 재계산은 **dry-run replay + 별도 반영**이라
>   원본 `Match.*RatingUpdate` 를 덮지 않는다 (D-171)
>
> ### 운영 실측 (2026-08-29 · 읽기만)
>
> - 감점 있는 선수 **773명** · 라플 TOP10 전원 마지막 경기가 **6/18~6/30** (시즌0 창 끝 7/1)
> - 3~5월에 멈춘 선수(감점 1,892 · 1,630 · 1,549)는 **표시 3000 · 무기 증감 합 0**
>
> ### ⚠ 꼭 알아야 할 것
>
> - **`SACLOUD_RATING_OWNER=formula` 가 없으면 30분마다 도는 `supply-rollup` 이 점수를 원본으로
>   되돌린다.** 실제로 그 사고가 났다. 두 워크플로에 이미 들어가 있다 —
>   `supply-incremental.yml` · `supply-rollup-full.yml`
> - ~~시즌0 창은 `2026-01-01 ~ 2026-07-01` 이다~~ → **D-175 로 `2026-04-01(KST) ~ 현재`(열린 구간)
>   로 바뀌었다.** 이 고정 창 때문에 7월 이후 경기가 집계에서 빠지고 있었다
>
> ### 이번 세션(이어받기)에서 추가로 한 일
>
> | | |
> |---|---|
> | `D-170~D-173` | 코드에는 인용돼 있는데 **`DECISIONS.md` 에 항목이 없었다** → 4건 작성 |
> | 테스트 | `isMain` 필터를 넣으면서 안 고친 무기 랭킹 테스트 **6건** 복구 + 회귀 2건 추가 |
> | 테스트 | `snapshotAuditSafety` 1건 — D-170 으로 비교 대상이 0 이 된 것. **이유가 미러 가드일 때만** 통과하도록 고침 |
> | `season0.ts` | 소스에 **NUL 바이트 2개**가 박혀 git 이 바이너리로 다뤘다 → `\u0000` 이스케이프로 교체(동작 동일) |
>
> **알려진 간헐 실패 2건** (둘 다 이번 변경과 무관 · 전체 1,208건 중 1건 안팎):
>
> 1. `independentLeague.test.ts` · `playerProfileIdentity.test.ts` 의 개인 상세가
>    로컬 DB 연결이 끊기며(`Can't reach database server`) 가끔 실패한다.
>    **연결 수 고갈은 아니다** — 테스트가 도는 동안 재 봤더니 최대 동시 연결 **12** 였다
>    (`max_connections` 100). 원인 미상 · **미해결로 남긴다.**
> 2. `health.test.ts` 의 `공개 데이터 수치가 실제 DB와 맞는다` — 다른 테스트 파일이
>    리그를 만들었다 지우는 사이에 재면 `before 4 · 조회 5 · after 4` 가 된다.
>    테스트 자신이 주석으로 인정한 교차 간섭이고 **예전부터 있던 것**이다.
