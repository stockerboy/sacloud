...
| **유저당 열어야 하는 페이지** | **1 ~ 리그 수** (아래 미확인 항목에 달렸다) |
| 총 페이지 | **5,000 ~ 7,500** |
| 페이지당 사람 작업 | 이동 + 스니펫 실행 + 붙여넣기 ≈ 15초 |
| **총 사람 작업 시간** | **20~30시간 (쉬지 않고)** |
...
- [ ] **한 플레이어의 지난시즌 페이지에 리그가 몇 개 나오는가** ← 작업량을 좌우한다
      카드에 리그 이름(`서플라이공식리그`)이 붙어 있다. 두 가지 가능성이 있다.
      - (a) URL의 리그 하나만 → **리그마다 페이지를 열어야 한다**
      - (b) 참여한 모든 리그가 한 페이지에 → **플레이어당 1페이지면 끝난다**
      → 아래로 스크롤해 **다른 리그 이름의 카드가 있는지** 보면 판별된다
- [ ] 시즌 1~5 카드도 같은 페이지에 있는지 (스크롤 아래)
