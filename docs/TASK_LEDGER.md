...
> 남은 것은 진행중 작업(`B14` 폼그래프 · `B15` 증분 5분) · 미착수(`B11`) ·
...
| B15 | **증분 동기화 5분** — 사용자: "5분 내로 뜨면 좋겠다" | **진행중** | 실측 병목 2개: ①사이클마다 클랜 **190개**를 훑어 새 경기 **평균 1.5건** 발견 ②집계 **전수 재계산 10~25분**. 적응형 폴링 + 증분 집계로 1분 내 목표 | 사용자 에이전트 | B8 워크플로의 cron 주기(현재 30분)도 함께 조정돼야 한다 |
| B16 | **UI 크기** — 모바일이 원본보다 큼 | **완료** | 모바일 루트 폰트 **16px → 14px** 회귀 수정(`packages/ui/src/styles.css`). 원본이 커 보인 것은 루트가 커서가 아니라 화면이 좁아 상대적으로 크게 보인 것이었고, 16px 로 올리자 rem 값이 전부 약 14% 커져 **원본보다 큰 화면**이 됐다. 원본은 PC·모바일 모두 루트 **14px**(D-009 실측). 접힌 매치 카드의 `1부리그 알수없음` 줄도 모바일에서 감췄다(원본에 없다) | 완료 | 미커밋 |
| B17 | **경기별 포지션** — 배틀로그 좌표로 경기마다 `2층`/`바닥`/`숏`/`리베`/`스나` 표시 | **막힘 — 사용자 회신 대기** | 사용자가 원본 배틀로그가 **맵 위에 킬 지점을 찍는 것**을 보여 줬다 → **좌표가 실재한다**. 경로를 몰라 요청을 엿듣는 스니펫(`packages/db/legacy/barracks-sniff-snippet.js`)을 사용자에게 전달, 회신 대기 | 사용자 | **수집은 브라우저만 가능하다(서버는 403).** 하루 440경기라 자동화하려면 **사용자 PC 러너**가 필요 → B15 와 묶일 수 있다 |
