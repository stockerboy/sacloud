...
> ~~운영에 배포된 코드는 `ba5aeab` 다~~ → **B9 해소됨 (2026-08-28 09:5x).**
> 전부 `bf83348` 로 커밋·푸시됐고 `main` 이 `origin/main` 과 같다. **Vercel 배포도 반영됐다.**
> 감독관이 운영 URL 로 재측정해 **B3 · B5 · B6 이 실제로 고쳐진 것을 확인했다** (10절).
>
> **남은 눈에 보이는 결함은 `B4` 하나다** — 개인랭킹 응답의 `clan` 이 아직 전부 `null` 이다.
> 원인이 확정됐다: **코드는 다 됐는데(D-160) 그 결과가 운영 DB 에 안 올라갔다.**
> 감독관 실측 — 운영 `LeaguePlayer.clanId` supply **113/10,388** · sanply **0** · daerule **0**
> (로컬은 2,635 / 3,719 / 820). **운영 DB 쓰기는 사용자 전용**이라 감독관이 할 수 없다.
> 그 외 남은 것은 `B8`(가동 결정) · `B11`(미착수) · 열린 질문(6절).
...
| B4 | 개인랭킹 닉네임 옆 클랜마크가 안 나온다 | **로컬 완료 / 운영 미반영 — 지금 유일하게 눈에 보이는 결함** | **D-160 으로 규칙 확정·구현 완료**(`packages/db/ops/supplyRollup.ts`). 규칙: 원본이 주는 `player.clan` 은 경기 당시가 아니라 **수집 시점의 현재 소속**이므로(실측 3,322명 중 경기별로 소속이 달라진 사람 **0명**) **가장 최근 경기 값 하나**를 옮긴다. 교차검증 — 수집 파일만으로 DB 없이 다시 구해 대조: 29,798행 중 **9행(0.03%)만 차이**, 전부 설명됨. 2회 실행 결과 동일(idempotent). **로컬 결과**: supply 2,635/10,388 · sanply 3,719/15,312 · daerule 820/4,157 · `Player.clanId` 4,419/21,107. **그러나 운영 DB 는 그대로다** — 감독관 실측: supply **113/10,388** · sanply **0** · daerule **0** · `Player.clanId(3rd.supply)` **0/21,107** | 사용자 에이전트 (코드 완료) | **운영 DB 반영이 남았다.** 운영 DB 쓰기는 사용자 전용이다(`supplyPush` 경로, D-156) |
...
| B9 | 커밋·배포 | **완료** | `git status --porcelain` = **71개 변경**(08:25 기준). `git branch -vv` = `main ... [ahead 9]`, `origin/main = ba5aeab`. **운영은 ba5aeab 를 돌리고 있다.** 이 하나가 B2·B3·B5·B6·B7·A4 를 전부 막고 있다 | **사용자 (완료)** | `bf83348` 로 커밋·푸시됨. `main == origin/main`. Vercel 배포 반영 확인 |
...
| D-163 | *(비어 있음 — 쓰지 말 것. 한때 잘못 할당됐다가 회수됨)* | — | — |
...
`D-162`(병영수첩 주소 파서) 와 `D-164`(래더 반영 경기 판정 일원화) 는 **코드가 인용하는데
`DECISIONS.md` 에 항목이 없다.** 각 작업 담당이 채워야 한다.
`CLAUDE.md` 3장 11번("중요한 프로젝트 지식과 결정은 문서화")에 걸린다.
