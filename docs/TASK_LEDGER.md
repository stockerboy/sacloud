...
> ⚠ **정정 (2026-09-02 밤 · 기록관)** — 이 절의 「다음 번호는 D-265」는 옛 서술이다.
> D-265 는 아래 「2026-09-02 밤」 절에서 썼다. **다음 번호는 D-266 이다.**

| 번호 | 주제 | 상태 |
|---|---|---|
| **D-264** | 색 3톤(op.gg 톤) · 선수/클랜 카드 개편 · IPL 킬뎃 top100 · 경기카드 승패 색 | ~~`DECISIONS.md` 에 아직 본문 없음 ⚠~~ → **본문 있음** (2026-09-02 밤 · 기록관 채움). 근거는 커밋 메시지 `b0c57f9` `6d0b423` `23f1e24` `96fe474` `91b1d9c` + `HANDOFF_2026-09-02_EVENING.md` 7장 |
...
|---|---|---|---|
| **D-260** | **연속킬**(선수 육각형 5번 축 · `burstChained`) | ~~**아직 없다 ⚠**~~ → **있다** (2026-09-02 밤 · 기록관이 코드 주석·테스트 근거만 옮김) | 코드는 여전히 작업트리에만 있고 커밋 안 됨 |
...
> ⚠ **정정 (2026-09-02 밤 · 기록관)** — 위 「다음 새 번호는 D-264」는 옛 서술이다. D-264 는 그 뒤
> 3톤/카드개편이 가져갔고(위 저녁 절), D-260 본문은 커밋을 기다리지 않고 **코드 근거만으로 채웠다**
> (코드 인용 8곳 — `playerBurst.ts` · 테스트 · `index.ts` · `schema.prisma` · `roundBuilderVersion.ts` ·
> `traits.ts` · `playerTraits.ts` · `roundBuild.ts`). 커밋 해시는 나오면 D-260 머리에 적는다.

### 2026-09-02 밤 — D-265 등록 · **다음 번호는 D-266** (기록관 · 총괄 지시 #5)

| 번호 | 주제 | `DECISIONS.md` | 상태 |
|---|---|---|---|
| **D-260** | 연속킬 (선수 육각형 5번 축) | **있다** (코드 근거만 옮김) | 코드 커밋 대기 |
| **D-264** | 색 3톤 · 선수/클랜 카드 개편 · IPL 킬뎃 top100 · 경기카드 승패 색 | **있다** | 운영 반영됨 (커밋 5개) |
| **D-265** | 2026-09-02 사장님 결정 3건(무료 수집 서버 · 넥슨 제휴 안 함 · IPL 1·2부 화면에서 없앰) + 팀 편성 + 소통(요청 대장) | **있다** | ①은 LANE E 시험 중 · ③은 LANE C 처리 대기 |

`grep -n "^## D-26[0-5]" docs/DECISIONS.md` 로 260 · 261 · 262 · 263 · 264 · 265 가 **각각 한 번씩**
나오는 것을 확인했다. **다음 새 번호는 `D-266` 이다.** 배정은 총괄이 한다 (`docs/TEAM_PIPELINE.md` 7장).

> ⚠ **정정 (같은 날 밤 · 기록관 · 총괄 지시 #5-2)** — 위 「다음 새 번호는 D-266」은 옛 서술이다.
> D-266 은 아래에서 썼다. **다음 새 번호는 `D-267` 이다.**

### 2026-09-02 밤 — D-266 등록 · **다음 번호는 D-267** (기록관 · 총괄 지시 #5-2)

| 번호 | 주제 | `DECISIONS.md` | 상태 |
|---|---|---|---|
| **D-266** | 병영수첩 수집 속도 상한 — delay 200ms · 296건 만에 집 IP 403 차단. 하한 1000ms · 기본 1500ms · 첫 403 즉시 정지 · concurrency 기본 1 · 재시도는 15분 뒤 1건 · 하루 상한 `[미확인]` | **있다** | 집 IP 차단 중 (풀리는 시점 모름). 런북 3장에 정정 상자 |

배정 전 확인: `git grep -n "D-266"` 결과가 「다음 번호」 언급 5줄뿐 — 코드 인용 0건이었다.
