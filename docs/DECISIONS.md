...
| 2026-08-21 | Phase 7 화면 검수 미완 | 리그 목록 표 · 리그 관리(소유자 화면) · 리그 만들기 · 통합검색 UI · 404 화면 · 작은 화면 레이아웃 | **스크린샷 미실시.** Chrome 창이 포그라운드가 아니어서 탭이 `visibilityState: hidden`으로 throttle됐다(D-019). 서버·API 수준은 확인했으나 **화면은 확인하지 못했다** |
