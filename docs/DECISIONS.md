...
### D-102. 무소속은 **계산 조건이 아니라 공개 범위 조건**이다 (2026-08-22 정정)
...
### D-107. `[정책 충돌 · 미해결]` 무소속 경기가 개인 래더를 올리는가

**정책 문서와 현재 코드가 다르다. 코드를 바꾸지 않고 사용자 판단을 기다린다.**

| 출처 | 내용 |
|---|---|
| 최신 정책 문서 16장 | "무소속 클랜 경기에서는 개인의 누적 공식 career 기록을 만들지 않는다. 공식 개인 rating·ranking·career win/loss·KD를 **올리지 않는다**" |
| 현재 코드 (D-102, 2026-08-22 정정) | 무소속 경기도 **개인 래더를 계산하고 저장한다.** 사용자 화면에서 **장기 누적만 숨긴다** |

D-102가 그렇게 정한 이유가 문서에 남아 있다 — 클랜 경기의 전력차를 계산하려면 실제 참가
선수의 실력값이 필요하고, 무소속 선수를 계산에서 빼거나 1500으로 고정하면
`무소속 A(라인업 평균 1700) vs 무소속 B(1450)` 경기의 예상 승률이 통째로 틀어져
**무소속 클랜의 래더까지 같이 틀린다.**

즉 두 문서는 "무소속 개인 커리어를 공개하지 않는다"에는 **일치**하고,
"그 경기가 개인 래더 숫자를 움직여도 되는가"에서 갈린다.

선택지

1. **현행 유지(D-102)** — 계산·저장하고 화면에서만 숨긴다. 클랜 래더 정확도가 지켜진다
2. **정책 16장대로** — 무소속 경기는 개인 래더에 반영하지 않는다.
   이때 클랜 래더의 라인업 전력 계산에 무엇을 쓸지 **함께 정해야 한다**
   (마지막 공식 래더? 1500 고정? 후자는 D-102가 틀렸다고 판단한 방식이다)

정하기 전까지 코드는 **1번(현행)** 이다. 임의로 바꾸지 않는다.
