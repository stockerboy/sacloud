...
```
클3+용2 vs 클3+용2  → 공식        클2+용3 vs 클2+용3  → 참고 기록
클3+용2 vs 클2+용3  → 공식        클2+용3 vs 클1+용4  → 참고 기록
클3+용2 vs 클1+용4  → 공식        클1+용4 vs 클1+용4  → 참고 기록
클3+용2 vs 클0+용5  → 공식
```
...
`Match.official = false`로 저장하고, 화면에 **`참고 기록 · 래더 미반영`** 배지를 보여 준다.
...
### D-095. 공식 ↔ 참고 전환은 비대칭이다

운영자가 잘못 인식된 경기를 바로잡을 수 있어야 한다 (정책 18).

- **공식 → 참고**: 바로 된다. 통계에서 빼는 방향이라 안전하다
- **참고 → 공식**: `reason`을 적어야만 된다. 근거 없이 통계에 넣지 않는다
...
| `nexon:check` | 인정 기준이 AND로 남아 있었다 | **공식 경기만** OR 조건 검사 + 참고 기록 래더 미반영 검사 추가 |
