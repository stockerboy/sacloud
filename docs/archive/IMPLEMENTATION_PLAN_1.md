...
**8) 진행 상태 — 2026-08-20 완료**
`typecheck` / `lint` / `test`(98건) / `next build` 통과, 원본과 Chrome 실브라우저 비교 검수 완료.
Playwright E2E와 반응형 스냅샷(1280/1024/390)은 하지 않았다 — 원본이 1120px 고정폭 데스크톱 전용이고
모바일 화면이 `[미확인]`이라 비교 기준이 없다. 사유는 `docs/DECISIONS.md` D-013.
실측 결과와 의도된 차이는 `docs/DECISIONS.md` D-009 ~ D-013 참조.
...
> **⚠ 2026-08-31 정정 — 바로 윗줄은 2026-08-21 판단이다. 지우지 않는다.**
>
> 그때는 사실이었다. 그러나 **2026-08-23 에 뒤집혔고, 계획 문서가 따라오지 않았다.**
>
> ```
> 그때  병영수첩 내부 API 는 쓰지 않는다 — 공식 Open API 하나만 쓴다
> 지금  공식 Open API 가 여전히 1차 출처다. 다만 넥슨이 안 주는 값 때문에
>       병영수첩 **BattleLog** 를 보조 출처로 함께 쓴다
> ```
>
> | 무엇 | 근거 |
> |---|---|
> | 라플/스나 판정은 **병영수첩 BattleLog 의 킬 신호**로 한다. Open API 에 무기 필드가 없기 때문이다 (`POST /api/BattleLog/GetBattleLogClan/{match_key}/{clan_no}`) | **D-114** (2026-08-23) |
> | 그 BattleLog 를 **어떻게 받는가** — 서버에서 부르면 **403(WAF)**, **브라우저에서 부르면 200**(로그인 불필요). 브라우저가 원문을 모아 파일로 내려받고 `nexon battlelog-import --file … --confirm` 으로 **원문 그대로 멱등 적재**한다 | **D-200** (2026-08-30) |
> | 그 경로의 한계 — 크롬 자동 다운로드 한도(실측 66번째 파일 이후 정지). 마지막 405건은 파일이 안 남았다 | **D-203** (2026-08-30) |
> | 좌표까지 원문 그대로 보관하고 격자 분포 → 포지션 판정에 쓴다 | **D-174** (`CLAUDE.md` 7장의 `battlelog-import` · `position-build` 명령) |
>
> **바뀌지 않은 것**: 우회는 하지 않는다. D-200 이 명시한다 — rate limit 을 우회하지 않고
> (요청 간격 320ms), CAPTCHA 를 풀지 않고, 비공개 영역에 들어가지 않는다 (`CLAUDE.md` 3-A 5번).
> 아래 「2) 넥슨이 주지 않는 값」의 **무기** 항목도 그래서 지금은 `null` 이 아니라
> **BattleLog 로 판정한다**(판정 불가면 `unknown`). 나머지 값은 여전히 `null` 이다.
...
**Phase 1 화면 복원 중 발견 (2026-08-20)**
- 통합검색에서 정확일치 결과가 없을 때의 안내 — 원본은 화면 전환 없이 그대로 머문다(관측). 토스트·문구 유무는 미확인
- 인기게시글 목록에서 댓글 수가 0인 글의 표기 — `[0]`을 붙이는지 아예 감추는지 관측하지 못함 (우리는 감춘다)
- 로딩·빈 목록·요청 실패 상태의 표시 — 원본은 SSR이라 관측 불가
- 상대시간의 `분` `달` `년` 단위와 1분 미만 표기 (관측된 것은 `시간` `일` 뿐) → `docs/DECISIONS.md` D-011
- 셀렉터 드롭다운의 hover 색 — 클래스명에서 역산했을 뿐 실측하지 못함 → D-009
- GNB `nav-active` / `league-nav-active`가 붙는 정확한 경로 매칭 규칙 (하위 경로 포함 여부)
- 로그인 상태일 때의 GNB 우측 유저 메뉴 구성 (Phase 6에서 확인)


**Phase 7 실제 API 구현 중 발견 (2026-08-21)**
- **매치 라인업·상세 스탯의 정렬 기준** — DB에 순서 컬럼이 없어 삽입 순서(red 로스터 → blue 로스터)로 두었다. 원본 기준 미확인
- **리그 목록의 대표 클랜 3개 선정 규칙** — "먼저 참여한 순"으로 두었으나 미확인
- **리그 목록 자체의 정렬 기준** — "개설이 오래된 순"으로 두었으나 미확인
- **자동완성(검색) 결과의 정렬 기준과 상한** — id 오름차순 · 10건으로 두었으나 미확인
- **클랜원 목록의 정렬 기준** — 가입 시각 컬럼이 없어 플레이어 id 오름차순
- **닉네임 중복 허용 여부** — 유일 제약을 걸지 않았다 (D-024)
- **댓글 수(`comment_count`)가 삭제된 댓글을 포함하는지** — Mock 픽스처가 포함해서 세므로 그대로 두었다
- **글 조회수 증가 규칙** — 같은 요청자의 연속 조회를 막는지 미확인. 지금은 단순 증가
- **댓글에도 글쓰기 rate limit(5분)이 걸리는지** — 지금은 글 작성에만 적용
- **공지 카테고리 작성 권한** — 운영자(role 2)만으로 제한한 것은 우리 결정
- **글 수정 시 카테고리 이동 가능 여부** — 지금은 원래 카테고리를 유지
- **익명 글에 작성자 본인 표시(`me`)를 주는지** — 비로그인 작성자 식별 수단이 없어 항상 false
- **서든어택 계정 연동의 실제 소유권 인증 방식** — 지금은 닉네임으로 연결만 하고 **소유권을 증명하지 않는다.** 운영 노출 전 반드시 해결해야 한다
- **`정보갱신`/`전적갱신`의 로그인·소유권 필요 여부와 재요청 제한**
- **클랜 설정 / 플레이어 설정의 소유권 판정 규칙** — 지금은 로그인 여부만 확인
- **클랜 승계 수락 절차** — 알림·대기 상태·만료 규칙 미확인. 지금은 요청만 기록하고 전적을 옮기지 않는다
- **리그 초대의 만료 기간과 수락 절차**
- **새 리그의 시작 시즌 번호** — 전역 현재 시즌으로 두었으나 원본이 1부터 시작하는지 미확인
- **로그아웃 엔드포인트 존재 여부** — 계약에 `authLogout`을 `designed`로 추가했다 (D-025)
...
- **UI 전면 교체 — 올블랙 · 고급스럽게 · 간결하게 · 한 화면을 꽉 채우지 않게.**
  버튼이 하는 일은 바꾸지 않는다. **3rd.supply UI 는 기능·컨텐츠를 담아 둔 임시 껍데기이고,
  모든 기능·작업이 끝난 뒤 통째로 갈아 끼운다** (`docs/DECISIONS.md` **D-188** · 사용자 확정).
  ⚠ 이 줄은 위 "미확정 아이디어" 와 성격이 다르다 — **사용자가 직접 지시한 확정 방향**이다.
  다만 착수 시점은 사용자가 정하고, 색값·강조색·타이포는 아직 정해지지 않았다.
  **미리 시안을 만들지 마라.**
