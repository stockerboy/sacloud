...
export const PlayerRankRow = z.object({
   ...
   *
   * ⚠ ★2026-09-18 — 줄 세우기는 `score_rating` 이 한다★ (사장님: «래더점수도 이걸로»).
   *   이 칸은 ★옛 Elo 래더★ 다. 지우지 않는다 (`CLAUDE.md` 1-4) — 되돌릴 때 재계산이 없어야 하고,
   *   아직 점수를 안 잰 선수는 이 값으로 줄을 선다.
   */
  rating: Rating,
  /**
   * ★점수 래더★ — 경기당 평균 점수 (2026-09-18 사장님).
   *
   * 값은 ★소수 한 자리★ 다 (20.9). DB 는 정수 칸(`scoreRating` = ×100)에 담고
   * 여기서 나눈다 — 나누는 곳이 하나뿐이라 헷갈릴 자리가 없다.
   *
   * ⚠ ★아직 안 잰 선수는 `null`★ 이다 — 0점으로 우기지 않는다 (D-106).
   *   `score_games` 가 문턱(20경기)에 못 미치면 그렇다.
   * ⚠ ★상위권 보정이 이미 들어 있다★ (위 11클랜 소속이면 경기당 +1.0).
   *   얼마를 받았는지는 `score_bonus` 가 따로 들고 있다.
   */
  score_rating: z.number().nullable().default(null),
  /**
   * 그 평균에 들어간 경기 수.
   *
   * ⚠ ★기본값을 둔다★ — 이 세 칸을 안 채우는 자리가 여럿이다(지난시즌·클랜별 목록 등).
   *   기본값이 없으면 그 자리마다 «없음» 을 손으로 적어야 하고, 하나만 빠뜨려도 통째로 깨진다.
   */
  score_games: Count.default(0),
  /** ★상위권 보정★ 으로 더해진 점수. 안 받았으면 0 */
