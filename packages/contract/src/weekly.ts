 ...
 *
 *   원문이 두 가지로 읽힌다 — 「수요일 00시」(화→수 경계) 와 「목요일로 넘어가는 경계」
 *   (수→목, 즉 목요일 00:00). **괄호가 뜻을 정한다고 보고 목요일 00:00 을 골랐다.**
 *   확인 중이니 값은 `WEEK_BOUNDARY.current` **한 곳**에만 있고, 다른 해석(`wed00`)도
 *   같은 표에 있어 한 줄로 바꿀 수 있다.
 *
 ...
 *   `wed00`  수요일 00:00 KST — 사장님 원문 「수요일 00시」를 글자대로 읽은 해석 (화→수 경계)
 *   `thu00`  목요일 00:00 KST — 괄호 「목요일로 넘어가는 경계」를 따른 해석 (수→목 경계)
 */
 ...
 *
 * 사장님 확인이 끝나 「수요일 00시」로 정해지면 `current: 'wed00'` 으로 바꾼다.
