...
/**
 * ★같은 무기끼리만 견주는 축★ (2026-09-12 사장님 → 2026-09-16 저녁까지 셋으로).
 *
 * > «스나싸움이랑 샷싸움만 라플끼리 스나끼리 비교해서 랭크매기고 나머지는 전부 통합»
 * > «(게임템포는) 스나수는 스나수끼리비교하고 라플수는 라플수끼리 비교해»
 * > «(크랙도) 이거 스나수랑 라플수 분리해서 매겨»
 *
 * ★워커와 화면이 이 목록 하나를 본다.★ 셈은 워커(`HEX_WEAPON_SCOPED_AXIS_KEYS`)가,
 * 이름은 화면(`poolNameOf`)이 붙이는데 ★목록이 두 벌이면 조용히 갈라진다★ —
 * 실제로 2026-09-16 밤에 갈렸다. 워커는 셋을 무기별로 매기는데 화면은 싸움만
 * 알아서 «스나수 171명중» 이어야 할 자리에 ★«통합 171명중»★ 이라 적혀 있었다.
 */
