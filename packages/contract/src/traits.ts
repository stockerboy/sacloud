...
> = {
   ...
   *
   * ⚠ ★2026-09-17 — 다시 «샷싸움» 으로 되돌렸다★ (사장님:
   *   «스나싸움 자리는 샷싸움 으로 넣어 라이플화력을 샷싸움으로만 바꾸는거야 내용은 같아»).
   *
   *   ★계산은 이번에도 한 글자도 안 바꿨다.★ 이름표만 왕복했다:
   *     샷싸움 → (2026-09-15 밤) 라이플화력 → (2026-09-17) 샷싸움
   *
   *   이렇게 되면서 개인 육각과 클랜 육각이 ★같은 이름을 쓰지 않게 됐다★ —
   *   9/15 에 노렸던 «한 낱말로 읽히게» 는 사라진다. 사장님이 그걸 알고 되돌리셨다.
   *   클랜 육각의 `riflePower` 는 ★손대지 않는다★ (뜻이 다른 축이다).
   */
  ...
  /**
   * ⚠ ★2026-09-17 — ④⑤⑥ 이 구역 축으로 바뀌었다★ (사장님 사양 원문):
   *
   *   > 스나6각: 세이브/소수싸움/스나싸움/a어택성공률/b어택성공률/A방어율
   *   > 라플6각: 세이브/소수싸움/샷싸움/어택성공률/방어율/크랙
   *
   *   ```
   *          바뀌기 전                 지금
   *   ④      기회창출 / 기회차단        ★A어택 / 어택성공률★
   *   ⑤      스나차이 / 라플차이        ★B어택 / 방어율★
   *   ⑥      안전함  / 크랙            ★A방어 / 크랙★      ← 라플은 그대로
   *   ```
   *
   *   라플의 어택성공률·방어율은 ★B + 2층 + 숏★ 셋을 합친 값이다. ★A 는 안 넣는다★ —
   *   A 단독은 앞선팀승률 70.3% 로 넷 중 꼴찌이고 선수 46.3% 가 분모 10 미만이다.
   *   빼면 「진 팀이 축을 이겨버린」 판의 설명력이 88.2% → 90.4% 로 오른다 (후보 열넷 중 1위).
   *
   *   ★옛 축의 재료도 셈도 안 지웠다★ — `TRAIT_AXIS_LABEL_V3` 에 이름을 남긴다.
   *   되살리려면 여기 세 줄을 그 값으로 되돌리고 `axisValuesOf` 의 세 칸을 되돌리면 된다.
   */
...
/**
 * ⚠ ★2026-09-17 까지 쓰던 ④⑤⑥★ — 지우지 않는다 (`CLAUDE.md` 1-4).
 * 옛 글·옛 화면이 이 이름을 말한다. 재료(`openRounds`·`cutRounds`·`aliveRounds`·`gapWinGames`)와
 * 셈(`axisValuesOf` 의 주석 처리된 갈래)도 그대로 살아 있다.
 */
export const TRAIT_AXIS_LABEL_V3: Partial<Record<TraitAxisAnyKey, { sniper: string; rifle: string }>> = {
