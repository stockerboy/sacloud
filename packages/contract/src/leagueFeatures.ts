...
function buildIpl(): Record<string, LeagueFeatureState> {
  ...
  /*
   * ⚠ ★래더도 그냥 «10/1 종료» 다★ (2026-09-16 사장님: «래더도 그냥 종료라고 해»).
   *
   *   내가 «래더는 처음부터 없으니 «미제공» 이 맞다» 고 따로 빼 뒀는데
   *   ★사장님이 되돌리셨다.★ 여덟 줄 중 하나만 회색이면 «이건 뭐가 다른 거지» 로
   *   눈이 멈춘다 — 보는 사람에게는 ★10/1 뒤에 못 쓴다★ 가 같은 말이다.
   *
   *   옛 판(래더만 «미제공»)은 이 주석이 기록이다 ( 1-4).
   */
  return out
}
