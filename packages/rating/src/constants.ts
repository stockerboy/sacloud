...
/**
 * ★★클랜은 따로 센다★★ (2026-09-21 · 사장님 화면에서 잡았다)
 *
 * ── 왜 따로 두나 (실측)
 *
 *   클랜 판수는 개인과 ★자릿수가 다르다★ —
 ...
 *   열산  중앙 ★ 9판★ · 75% 25판
 *   ```
 *   개인 기준(150판)을 그대로 쓰면 PL 클랜의 절반이 신뢰 0.4 로 눌려
 *   ★2승2패도 80승40패도 다 3,000점★ 으로 뭉친다 (사장님 화면에서 그렇게 보였다).
 *
 * ── 40 인 이유
 *   PL 중앙(24판)이 신뢰 0.77 로 남는다 — ★적당히 눌리되 뭉치지는 않는다.★
 *   이미 `iplClanStanding` 의 판수 수축(`CLAN_SHRINK_GAMES`)도 40 이라 잣대가 같다.
 */
export const CLAN_CONFIDENCE_FULL_AT = 40
...
export const V2_RATING_CONSTANTS: RatingConstants = {
  ...
  v2: {
    ...
    /*
     * ★★2026-09-21 — 판수 신뢰를 다시 켰다★★ (사장님: 「★판수신뢰 키고★」)
     *
     * ── 왜 껐었나 (2026-09-14)
     *   「판수가 순위를 올리지 못하게」 하려고 껐다. 같은 실력인데 판만 많은 사람이
     *   위에 오는 것을 막으려던 것이다.
     *
     * ── 왜 다시 켜나 (2026-09-21 실측)
     *   문턱(40판)을 걷어 ★전부 공개★ 로 바꾸자 ★판 적은 사람이 위로 올라왔다★ —
     *   ```
     *   PL   5위 doping  ★16판★ 93.8%   →  3,829점
     *   열산 7위 ㅇ1ㅇ    ★16판★ 100%    →  3,927점
     *   ```
     *   사장님이 늘 싫어하신 그림이다 (「몇판 하지도 않은 애들이 100위 안에 있는 게 젤 싫어」).
     *
     * ── ★보정이 아니다★
     *   상위권 보정·티어 보정과 다르다. ★판이 적으면 아직 못 믿는다★ 는 뜻이라
     *   기준점(3000) 쪽으로 눌러 두고, ★150판을 채우면 제 점수를 다 준다.★
     *   아무도 숨기지 않는다 — 전부 공개는 그대로다.
     *
     *   실측으로 본 결과 (판수 신뢰를 켜면)
     *   ```
     *   doping   16판  3,829 → 3,271     차값   103판  4,064 → 3,882
     *   헐떡이    27판  3,871 → 3,369     다혁힝  90판  4,488 → 4,152
     *   ```
     *
     * ⚠ 되돌리려면 이 줄을 `true` 로 둔다 (`CLAUDE.md` 1-4).
     */
    disableDisplayConfidence: false,
