...
export function roundTallyOf(input: {
  ...
  for (const state of roundStatesOf(input.events).values()) {
    ...
    /*
     * ★그 라운드를 이겼는지 모르면 아예 안 센다★ (2026-09-15 사장님이 화면에서 잡아 주심).
     *
     * ⚠ 옛 판은 `won` 이 `null` 이어도 ★분모만★ 늘렸다. 그러면 «혼자 남았는데 못 이김»
     *   으로 쌓여서, 승패를 모르는 경기가 통째로 «세이브 0/15» 가 됐다.
     *   실측(260915013708124001): 개인 0/15 인데 클랜은 4/10·1/10 이었다 —
     *   ★클랜 쪽에는 이 조건이 있었고 개인 쪽에만 빠져 있었다★
     *   (`clanHexV2.ts` 의 `won !== null`).
     *
     * 모르는 것을 0 으로 적지 않는다 (D-106).
     */
