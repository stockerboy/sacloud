...
describe('roundTallyOf — 세이브 · 소수싸움 재료', () => {
  ...
  /**
   * ⚠ ★2026-09-15 뒤집었다★ — 사장님이 화면에서 잡아 주셨다:
   *   «양팀 다 아무도 세이브 한적이 없는데 40:10으로 뜨는 이유가 궁금해»
   *
   *   옛 규칙은 «승패를 모르는 라운드도 ★분모에는★ 남는다» 였다 (바로 아래 옛 시험).
   *   그러면 «혼자 남았는데 못 이김» 으로 쌓여서, 한 경기가 통째로 «세이브 0/15» 가 된다.
   *   실측: 개인 세이브 4,259/63,659(6.7%) · 클랜 세이브 51,095/352,816(14.5%) —
   *   ★같은 것을 재는데 두 배 차이★ 였다. 클랜 쪽(`clanHexV2.ts`)에는 `won !== null`
   *   조건이 있었고 개인 쪽에만 빠져 있었다.
   *
   *   모르는 것을 «못 했다» 로 적지 않는다 (D-106).
   */
  it('★승패를 모르면 분모에도 안 넣는다★ (2026-09-15 사장님)', () => {
    const events = [
      ...fullRoster(),
      kill(1, 10, B(1), A(2)),
      kill(1, 20, B(1), A(3)),
  ...
  /* 옛 규칙의 시험 — 되돌릴 때를 위해 남긴다 (CLAUDE.md 1-4)
