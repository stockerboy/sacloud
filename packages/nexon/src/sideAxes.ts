 ...
 *
 * ★날숫자 비중 그대로 견준다★ — 자리별 인구 중앙값으로 정규화하지 않는다.
 *   정규화하면 2층이 부푼다 (2층 구역이 67칸으로 좁아 인구 비중이 15.7%뿐인데
 *   그걸로 나누니 26.9%가 됐다. 사양은 20%다). 실측에서 두 방식이 갈리는 132명(6.6%)이
 *   ★전부 한 방향★ 이었다 — 2층→비리베 59 · 2층→숏 55 · 숏→비리베 18.
 *
 * 경계 1.1 은 사장님 채점 세 줄에서 ★오답 0★ 이 나오는 값이다 (2026-09-17):
 *   applemango. 1.18 → 2층 ✅ · 김철영 1.09 → 올포지션 · 불개미321 1.06 → 올포지션
 *   두 오답 후보는 배수가 낮아 ★숫자로는 못 가르는★ 선수들이다. 사장님이 맞히신 건
 *   그 선수를 아셔서지 값이 말해 준 게 아니다. ★모르면 이름을 안 붙인다.★
 *   1.1 이면 30경기 이상 1,985명 중 104명(5.2%)이 올포지션이다.
 *   ⚠ 옛 값은 1.3 이었다 (255명·12.8%).
 */
export const POSITION_MIN_RATIO = 1.1
/** 자기 킬의 이만큼이 스나면 스나다 */
