...
/**
 * ★크랙★(라플) — 정해 둔 구역을 25초 안에 뚫는다 → 갈라진 벽.
 *
 * ⚠ 첫 판은 ★번개 모양 획★ 하나였는데 22px 에서 그냥 ★X 자★ 로 보였다.
 *   금은 ★무언가가 갈라져야★ 읽힌다 — 흰 벽을 깔고 그 사이를 판다.
 */
function GlyphCrack({ carve }: CarveProps) {
  return (
    <>
      ...
      {/*
       * ⚠ ★세 번 그렸다★ — 찍어 보고 두 번 버렸다
       *   ① 번개 모양 획 하나        → 22px 에서 그냥 «X 자»
       *   ② 넓은 틈으로 크게 꺾음    → 남은 흰 조각이 글자 «Ŋ»
       *   ③ 벽돌 줄눈까지 파 봄      → 흰 벽이 여덟 조각으로 부서져 «창틀»
       *   남은 것이 지금 것이다 — ★벽 하나 · 금 하나 · 잔금 둘.★
       *   흰 면이 크게 남아야 «벽» 으로 읽히고, 그래야 금이 «금» 이 된다.
       */}
      <path
