 ...
 *
 * ⚠ **2026-09-02 (O-006) — 게시판을 이 줄에서 뺐다. 성격이 다른 둘을 묶고 있었다.**
 *
 * ```
 * 본인인증 · 소유권 증명   진짜로 **없다.** 만든 적이 없다
 * 게시판                  없는 게 아니라 **길이 막힌** 것이다 —
 *                        /board/free 는 200 이고 글쓰기 버튼도 있는데(D-260),
 *                        홈에서 거기로 가는 링크가 한 개도 없다 (GNB 에서 감춤)
 * ```
 * 둘을 한 줄에 묶으니 게시판 쪽이 거짓말이 됐다. 줄을 가르고, 게시판은 아래
 * `BOARD_PREPARING` 한 줄로 따로 말한다.
 *
 * (O-001 때 여기 넣었던 판단은 「닫혀 있다」는 잘못된 전제 위에 있었다.)
 */
const NOT_YET =
  '아직 없는 것: 본인인증과 계정 소유권 증명, 그리고 경기 로그가 더 쌓여야 나오는 일부 클랜 지표.'

/**
 * 게시판 한 줄 — **`BOARD_OPEN` 이 거짓일 때만** 그린다 (O-006 · 2026-09-02).
 *
 * ★스위치 하나가 둘을 같이 뒤집는다★ — 게시판을 열면 이 줄이 사라지고,
 * 그 자리에 사장님이 쓰신 「게시판」 문단이 나온다. 두 곳을 따로 고칠 일이 없다.
 *
 * **날짜를 적지 않는다.** 언제 여는지 정해진 것이 없다 — 지어내면 그날 거짓말이 된다
 * (`boardPreparingText.ts` 와 같은 이유).
 */
const BOARD_PREPARING = '게시판은 준비 중입니다.'
  ...
  return (
    <section className="mx-auto w-full max-w-[720px] border-t border-line pt-8">
      ...
      <div className="text-[14px] leading-relaxed text-meta">
        ...
        {/* 게시판은 「없는」 게 아니라 「길이 막힌」 것이라 줄을 따로 둔다 (O-006).
            열면 이 줄이 사라지고 위의 「게시판」 문단이 대신 나온다 */}
        {BOARD_OPEN ? null : (
          <p className="mt-1 text-[12px] text-[var(--color-faint,#6b5555)]">{BOARD_PREPARING}</p>
        )}
      </div>
