 ...
 *
 * ⚠ ★★7 이었는데 첫 점의 값 글자가 Y축 숫자와 겹쳤다★★ (2026-09-04 · 폰 390px 실측).
 *
 * ```
 * 왼쪽 축 자리   숫자칸 24px + 사이 8px = ★32px★
 * 카드 안쪽 폭   390 − 카드 여백 32 ≒ ★358px★
 * 옛 여백 7%     358 × 0.07 = ★25px★  ← ★32px 보다 좁다. 그래서 겹쳤다★
 * 새 여백 14%    358 × 0.14 = ★50px★  ← 값 글자(≒28px)의 절반을 빼도 36px 로 남는다
 * ```
 * ★오른쪽도 같이 넓어져 마지막 점의 값 글자가 잘리지 않는다.★
 * ★옛 값은 아래에 남긴다★ (`CLAUDE.md` 1-4).
 */
const PAD_X = 14
/** ★옛 여백★ — 첫 점 글자가 축 숫자와 겹쳤다. 되돌리면 그 문제가 돌아온다 */
export const PAD_X_V1 = 7
const PAD_TOP = 12
const PAD_BOTTOM = 10
