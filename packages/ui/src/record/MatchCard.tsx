          ...
          <div className="flex items-center px-2 pt-1.5 text-sm text-meta">
            ...
            */}
            ...
            <div className="ml-auto shrink-0 pl-2 font-semibold">
              {/* 이 창에 0판이면 래더 증감 대신 `기록 없음` (`PLACEMENT_LABEL`).
                  옛 문구는 「배치고사」였고 그 모드는 2026-09-01 에 폐지됐다 */}
              {match.placement ? (
                PLACEMENT_LABEL
...
function StatRow({
  ...
  return (
    ...
    >
      ...
      {/* 마지막 칸 (D-250)
          기본(`holo`)은 **이 경기 래더 변동**이다. 부호를 반드시 보이고,
          `placement` 면 숫자 대신 `PLACEMENT_LABEL` 을 쓴다 (0 으로 쓰면 "0점 움직였다" 가 된다).
          값이 없으면 `알수없음` — 지어내지 않는다.

          ⚠ 여기 「배치고사 중이면 숫자 대신 `배치고사` 를 쓴다」고 적혀 있었다 (2026-09-03 정정).
            ★배치고사는 2026-09-01 에 폐지됐고★ 이 글자는 `O-038` ③ 에서 「기록 없음」이 됐다.
            `placement` 플래그는 남았지만 **뜻이 바뀌었다** — 「이 창에 0판」이다.
