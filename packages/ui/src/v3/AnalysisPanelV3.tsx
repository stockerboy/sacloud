...
const AXES: readonly (readonly [string, string])[] = [
  ...
  /*
   * ⚠ 이름이 왕복했다 — 샷싸움 → (2026-09-15 밤) 라이플화력 → (2026-09-17) 샷싸움.
   *   설명은 ★무기마다 다르다★ — 스나는 «롱 안 스나 대 스나», 라플은 «라플끼리».
   *   여기 표는 이름 하나에 설명 하나뿐이라 ★둘을 아우르는 말★ 로 적는다.
   *   ⚠ «라이플화력» 이 적힌 옛 글·옛 화면이 있으므로 그 줄도 남겨 둔다.
   */
  ['샷싸움', '같은 무기끼리 붙어 이긴 비율입니다'],
  ['라이플화력', '같은 무기끼리 붙어 이긴 비율입니다'],
  ['게임영향력', '팽팽하거나 밀릴 때 따낸 킬만 셉니다 — 이미 이긴 판에서 딴 킬은 안 셉니다'],
  ['선짤(1턴)', '라운드 시작 25초 안에 첫 킬을 냈는지 봅니다'],
  ['백어택성공률(2턴)', '동료가 죽은 직후 5초 안에 그 킬러를 되잡은 비율입니다'],
