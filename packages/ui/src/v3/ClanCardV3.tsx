  ...
  return (
    <>
      {markSlug && hasFitMark(markSlug) ? (
        <span aria-hidden style={{ position: 'absolute', left: 0 /* 2026-09-23 — 옛 '-4%' 는 폰에서 화면 왼쪽 밖으로 2px 나갔다(운영 실측). 상자는 0, 그림만 -4% */, top: '8%', width: '58%', height: '112%', backgroundImage: `url(${fitMarkUrl(markSlug)})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundPosition: '-4% center', opacity: 0.05, filter: 'grayscale(1)', pointerEvents: 'none' }} />
      ) : null}
          ...
          {kpis.map((k) => (
            <div key={k.label} className="v3-clankpi" style={kpiRowStyle}>
              ...
              {/*
                ...
                *   폰 390px 에서 ★250px 떨어져★ 서 있었다. 기울어진 줄(`row-reverse`)에서는
                *   `flex-start` 가 ★오른쪽 끕★ 이다 — 반대로 준 셈이다.
                *   라벨 바로 옆에 붙이려면 `flex-end` 가 맞다.
                *   여전히 값끼리는 같은 자리에서 시작하므로 줄이 알맞게 맞는다.
                */}
              <span style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'flex-end', gap: 8, minWidth: 0, flexDirection: KPI_SPREAD ? 'row' : 'row-reverse' }}>
