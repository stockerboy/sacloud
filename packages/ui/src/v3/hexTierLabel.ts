...
const STEPS: readonly [number, string, string][] = [
  ...
  [20, '중상위권', '#0e86c4'],
  /*
   * ⚠ ★2026-09-22 — 실측하니 흰 카드에서 거의 안 읽혔다★. 다크 카드에서는 이
   *   회색들이 "차분하지만 읽히는" 세기였는데, 흰 바탕에서는 대비가 확 떨어져
   *   거의 안 보였다(라이브 화면 확인). 뜻(아래로 갈수록 옅어진다)은 그대로 두고
   *   진하기만 올린다. 옛 값(다크용)은 지우지 않는다 (`CLAUDE.md` 1-4):
   *   중위권 #9aa6bf · 중하위권 #7a869e · 하위권 #68738a · 최하위권 #5a6478
   */
  [50, '중위권', '#5c6479'],
  [75, '중하위권', '#767f96'],
  [90, '하위권', '#96a0b5'],
