...
/**
 * ⚠ ★2026-09-14 — 이 리그 이름이 ‘YSL’ 로 바뀌었다★ (사장님이 새 로고와 함께 정하셨다).
 *
 *   ★‘YSL’ 은 일부러 위 목록에 안 넣었다.★ 새 로고(`league-ysl.png`)에 산이 이미
 *   그려져 있어서, 이름 옆에 산 표시를 또 붙이면 ★산이 두 번★ 나온다.
 *   옛 이름(‘10’ · ‘10mountain’)은 그대로 알아본다 — 캐시나 목 자료에 남아 있을 수 있고,
 *   그때는 로고가 없는 자리라 산 표시가 이름 노릇을 한다.
 */
...
export function MountainMark({ className = '' }: { className?: string }) {
  return (
    ...
    >
      {/*
        2026-09-02 (지시 #14 ①) — 「귀여운 산」. 꼭짓점을 둥글리고 봉우리 둘을 나란히 세웠다.
        예전 모양(뾰족한 삼각 둘 · 2026-09-01)은 아래 주석에 남긴다 (`CLAUDE.md` 10-4).
          <path d="M10.6 5.4 15.4 13H10z" opacity="0.5" />
          <path d="M5.6 3 11.2 13H0z" />
          <path d="M5.6 3 7.2 5.8 6.3 5.4l-.9.6-.8-.6-.6.3z" opacity="0.45" />
        색은 여전히 `currentColor` 하나다.
      */}
      {/* 뒤쪽 작은 봉우리 — 둥근 꼭짓점 */}
      <path d="M10.9 5.9c.4-.6 1.2-.6 1.6 0l3 4.9c.4.6-.1 1.4-.8 1.4H9.4l1.5-6.3z" opacity="0.55" />
      {/* 앞쪽 큰 봉우리 — 둥근 꼭짓점 · 밑은 살짝 퍼진다 */}
      <path d="M5.3 3.3c.5-.7 1.5-.7 2 0l5 7.7c.5.7 0 1.6-.8 1.6H1.6c-.8 0-1.3-.9-.8-1.6z" />
      {/* 눈 모자 — 밝게 */}
      <path d="M6.3 4.4 7.9 6.9c-.4.3-.8.2-1.1-.1-.3.3-.7.4-1.1.1-.3.2-.7.3-1 0z" opacity="0.5" />
    </svg>
  )
}
