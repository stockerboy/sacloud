...
function axis(over: Partial<PlayerTraitAxis> = {}): PlayerTraitAxis {
  return { key: 'chance', label: '게임영향력', percentile: null, pending: 'rounds', ...over }
}
...
describe('pendingSummary — 아래 한 줄 요약', () => {
  ...
  it('이유가 중복되면 한 번만 적는다', () => {
    const axes = [
      ...
      axis({ key: 'chance', percentile: 71.2, pending: null }),
      axis({ key: 'safe', label: '미정', pending: 'undecided' }),
