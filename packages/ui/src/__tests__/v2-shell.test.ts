...
describe('탭을 시안에 맞춘다고 지우지 않았다', () => {
  ...
  /**
   * ⚠ ★2026-09-14 — TOP5 탭이 늘었다★ (사장님: «추가로 페이지 하나 더 만들자
   *   여기서는 클랜 , 개인6각 top5 보여주자 각 분야별 top5»).
   *   옛 기대값 — `['클랜랭킹', '개인랭킹', '경기']`
   */
  it('LLM 은 홈·클랜랭킹·개인랭킹 셋뿐이다 (2026-09-15)', () => {
    /*
     * ⚠ ★같은 날 두 번 바뀌었다★
     *   ① 맨 앞에 「홈」이 붙었다 (사장님: «각 리그 페이지에 홈(여기에 최근경기랑
     *      깃발 그래프 다 나옴) 그리고 이제 클랜랭킹 개인랭킹 이런식으로»)
     *   ② ★TOP5 와 경기가 빠졌다★ (사장님: «각리그 홈에다가 top5를 합쳐줘 /
     *      그리고 top5랑 경기페이지는 없애버려») — 둘 다 홈 안으로 들어갔다
     *
     *   옛 기대값 — `['홈','클랜랭킹','개인랭킹','TOP5','경기']`
     *   그 전 — `['클랜랭킹','개인랭킹','TOP5','경기']` · 더 전 — `['클랜랭킹','개인랭킹','경기']`
     */
    /* ⚠ 옛 기대값 첫 탭은 `'홈'` — 2026-09-15 밤 사장님 «홈 <버튼을 최근경기로 바꿔» */
    expect(leagueTabs('supply').map((t) => t.label)).toEqual(['최근경기', '클랜랭킹', '개인랭킹'])
  })

  /**
   * ⚠ ★2026-09-13 뒤집혔다★ (사장님: «세 리그 전부 공평하게 대한다»).
   *   옛 시험 — `['개인랭킹', '경기']` (클랜 탭이 없었다).
   */
  /**
   * ⚠ ★2026-09-14 또 뒤집혔다★ — 사장님: «열산은 클랜 기록 미제공».
   *   9/13 «세 리그 전부 공평하게» 로 열었다가 하루 만에 닫는다.
   */
  it('YSL 은 클랜 탭이 없다 — 클랜 기록을 안 준다 (2026-09-14)', () => {
    /*
     * ⚠ 옛 기대값 — `['홈','개인랭킹','TOP5','경기']` (2026-09-15 에 TOP5·경기가 빠졌다)
     * ⚠ 2026-09-20 ★세 번째 탭이 붙었다★ (사장님: 「열산리그에 세번째파트로
     *   고용가능클랜 으로 넣어」). ★클랜랭킹은 여전히 없다★ — 이 시험의 알맹이다.
     */
    expect(leagueTabs('sanply').map((t) => t.label)).toEqual([
      '최근경기',
      '개인랭킹',
      '고용가능클랜',
    ])
    /* ★클랜랭킹 탭은 끝까지 없다★ — 클랜 기록을 안 주는 리그다 */
    expect(leagueTabs('sanply').map((t) => t.label)).not.toContain('클랜랭킹')
  })

  it('★고용가능클랜은 10산에만 있다★ (2026-09-20 사장님)', () => {
    for (const slug of ['supply', 'nolink']) {
      expect(leagueTabs(slug).map((t) => t.label)).not.toContain('고용가능클랜')
    }
  })
