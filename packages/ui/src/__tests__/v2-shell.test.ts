...
describe('탭을 시안에 맞춘다고 지우지 않았다', () => {
   ...
   *   옛 기대값 — `['클랜랭킹', '개인랭킹', '경기']`
   */
  it('SPL 은 홈·클랜랭킹·개인랭킹·TOP5·경기', () => {
    /* ★맨 앞에 「홈」이 붙었다★ (2026-09-15 사장님: «각 리그 페이지에 홈(여기에
       최근경기랑 깃발 그래프 다 나옴) 그리고 이제 클랜랭킹 개인랭킹 이런식으로»).
       옛 기대값 — `['클랜랭킹', '개인랭킹', 'TOP5', '경기']` */
   ...
   */
  it('10 은 클랜 탭이 없다 — 클랜 기록을 안 준다 (2026-09-14)', () => {
    /* TOP5 는 리그를 안 가린다 — 그 화면 안에서 개인만 보인다 */
    expect(leagueTabs('sanply').map((t) => t.label)).toEqual(['홈', '개인랭킹', 'TOP5', '경기'])
  })
